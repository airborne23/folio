...
[mul-963]: https://folio-api.copilothub.ai/issues/28c34ad2-102a-4f46-91ac-336ed78c5859
...
If you see `no such host` *inside a Codex session on macOS* but `curl https://folio-api.copilothub.ai` from a plain shell on the same machine works, you are hitting the Seatbelt bug below.
...
The daemon writes a *folio-managed* block into each task's
`$CODEX_HOME/config.toml`, delimited by `# BEGIN folio-managed` /
`# END folio-managed` markers. Anything outside the markers is left
...
`[permissions.folio]`, a bare `sandbox_mode = "..."` line would be parsed
as `permissions.folio.sandbox_mode` and Codex would silently ignore it.
...
```bash
# Is the Folio API reachable at all?
curl -sSf https://folio-api.copilothub.ai/healthz
```

From inside a Codex session (after the daemon writes its config):

```bash
folio issue list --limit 1 --output json >/dev/null && echo OK
```
...
- **A. Domain-scoped `permissions` profile** (tight): when the upstream
  `network_access` fix is available, prefer writing a `permissions.folio`
  profile that allows only `folio-api.copilothub.ai` and
  `folio-static.copilothub.ai`. Keeps filesystem sandbox intact.
...
sed -n '/# BEGIN folio-managed/,/# END folio-managed/p' \
  ~/folio_workspaces/$WORKSPACE_ID/$TASK_SHORT/codex-home/config.toml
```
