...
This document is the source of truth for the analytics events Folio ships
...
See [MUL-1122](https://github.com/folio-ai/folio) for the design context.
...
Self-hosters should **never inherit a Folio-issued `POSTHOG_API_KEY`** —
...
- **Person properties (`$set`)** — use for mutable cohort signals
  ...
  `setPersonProperties()` in `@folio/core/analytics`. Use
...
| `signup_source` | string | Opaque attribution bundle from the frontend cookie `folio_signup_source` (UTM + referrer). Empty when the cookie is absent. |
...
| `cli_version` | string | Version of the `folio` CLI that registered it. |
...
- Attribution is NOT a separate event; UTM + referrer origin are written
  to the `folio_signup_source` cookie on the first anonymous pageview
