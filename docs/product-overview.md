...
> **文档说明**
>
> 这份文档的目的是：**让任何没有写过代码的新同事，在 30 分钟内完全理解 Folio 这个产品到底有哪些功能、每个功能在整体中处于什么位置、一个功能和另一个功能如何协同**。
>
> 它的受众包括：
>
> - **新加入的工程师 / 产品 / 设计 / 运营**——用它做 onboarding 的第一份材料
> - **产品介绍工作**——需要对外讲解 Folio 时的事实基础
...
> Folio 不是一个 AI 工具，而是一个**人 + AI 协作的任务管理平台**。agent 是一等公民，和人在同一个工作流里。

### 部署形态

- **云版本（Folio Cloud）**：官方托管服务，agent 通过你本地跑的 daemon 执行
- **自托管（Self-Host）**：完整后端可以部署在自己的服务器
- **客户端**：Next.js web 版 + Electron 桌面版（两端体验基本一致，桌面独有：多标签、原生托盘、自动更新）

### 支持的 Coding Agent

Folio **不自己训模型**，也不锁定某一家厂商。它是调度器，本地 daemon 会自动探测以下 CLI 工具并接入：
...
Issue 对应的概念在 Linear 叫 Issue、在 Jira 叫 Ticket、在 GitHub 叫 Issue——就是一个任务单元。Folio 的特色在于**issue 可以分配给 agent，和分配给人完全对等**。
...
Agent 是 Folio 的灵魂。几乎所有功能都围绕"如何让一个 agent 干活"展开：
- Issue 通过分配触发 agent
- Skill 通过挂载赋能 agent
...
这是 Folio **分布式执行架构**的核心设计：**agent 不在 server 上运行，而在用户自己的机器上运行**。Server 只做任务调度、状态同步、数据存储。

#### Daemon 是什么

`folio` CLI 在用户的机器上启动一个后台进程（macOS launchd / Linux systemd / Windows 服务风格），它：
...
| 命令 | 说明 |
|------|------|
| `folio setup` | 一键配置：填 URL + 登录 + 启动 daemon |
| `folio login` | 浏览器打开 OAuth 登录，保存 90 天 PAT 到 `~/.folio/config.json` |
| `folio login --token <pat>` | 无头登录（SSH/CI） |
| `folio daemon start` | 后台启动 daemon（写 PID 到 `~/.folio/daemon.pid`，日志到 `~/.folio/daemon.log`） |
| `folio daemon stop` | 发 SIGTERM，优雅关闭（等待进行中的任务完成，超时 30s） |
| `folio daemon status` | 打印 daemon 状态、探测到的 agent、watch 中的 workspace |
| `folio daemon logs -f` | 实时跟随日志 |
| `folio daemon start --profile <name>` | 启动独立配置的 daemon（用于多环境，比如同时连 staging 和生产） |

#### 安全边界

- 每个任务一个**独立工作目录** `~/folio_workspaces/{ws}/{task_short_id}/workdir/`
- 环境变量**过滤**：阻止 agent 覆盖 daemon 的认证变量（`FOLIO_TOKEN` 等）
...
folio skill list
folio skill get <id>
folio skill create --title ...
folio skill import --url https://...
folio skill files upsert <skill-id> --path ...
```

#### 产品里的位置

Skill 是 Folio 区别于"每次都要写长 prompt"的关键机制。它让团队的专业知识**沉淀成可复用的组件**，绑在 agent 上就生效——就像给员工写的 SOP/playbook。
...
- **API / Manual**：UI 上点"立即运行"按钮，或用 CLI `folio autopilot trigger <id>`
...
Autopilot 让 Folio 从"你分配 → agent 做"升级到"agent 自己发起工作"。配合 `run_only` 模式，甚至可以在没有 issue 的前提下跑定时任务。Issue 上的 `origin_type=autopilot` + `origin_id` 字段留下了"这个 issue 是哪个 autopilot run 创建的"的追溯链。
...
桌面端的 `folio://invite/{id}` 深链接**不是走路由**，而是触发 `WindowOverlay`——共享视图组件 `InvitePage` 装在原生窗口覆盖层里，保证拖拽移动窗口等原生体验。

#### 产品里的位置

成员管理是**一切协作的前提**。但在 Folio 里它有一个独特之处：成员系统也管 agent。之所以要有 `assignee_type` 区分 member 和 agent，就是为了让两者在同一套 API 里表达"谁可以被分配"。
...
Settings 是所有"配置即工作"动作的汇总：agent 的 prompt、workspace 的 context、仓库白名单、skill 的内容——都在这里。**对运营和文案来说最重要的一句话**：用户在 Folio 的 settings 页面做的配置，每一项都会影响 agent 实际执行时读到的上下文。

---

### 3.14 CLI 命令行工具

`folio` 不只是启动 daemon 的工具，也是完整的命令行操作层。很多用户喜欢在终端里推进工作而不是开 UI。

#### 工作区 / 议题

```bash
folio workspace list | get | watch | unwatch
folio issue list | get | create | update | assign | status
folio issue comment list | add | delete
folio issue runs <id>                 # 查看任务执行历史
folio issue run-messages <task-id>    # 查看某次执行的消息
```

#### Agent / Skill / Autopilot / Project / Repo

```bash
folio agent list | get | create | update | archive
folio skill list | get | create | update | delete | import | files upsert
folio autopilot list | get | create | update | trigger
folio autopilot trigger-add --cron "0 9 * * 1-5"
folio project list | get | create | update
folio repo list | add | update | delete
```

#### Runtime

```bash
folio runtime list | usage | activity | update
```

#### 配置 / 更新

```bash
folio config show | set server_url ...
folio auth status | logout
folio version | update
```

#### 产品里的位置

CLI 是 Folio 对开发者友好度的体现。对于 agent 自己来说，也同等重要——**agent 在执行任务时能调用 `folio` 命令读写 issue、评论、查文档**，这正是 CLI 在 "agent 作为一等公民"架构里的作用。
...
```
┌─────────────────────┐        ┌────────────────────┐        ┌──────────────────┐
│  Next.js Web App    │        │  Electron Desktop  │        │  folio CLI     │
...
| **Agent CLI（Claude Code 等）** | 实际调用 LLM、执行工具调用、写文件、跑测试 | 不感知 Folio 的数据模型（所有上下文通过 `folio` CLI 命令读回） |
...
**Folio 本身不直接调 LLM API**。所有 LLM 调用都在 agent CLI 子进程里发生（Claude Code 调 Anthropic API、Codex 调 OpenAI API 等）。
...
- **Invite accept overlay**（来自 `folio://invite/{id}` 深链接）
...
- **深链接**：`folio://auth/callback?token=...` 和 `folio://invite/{id}`
...
Folio 的设计可以归结为一句话：**把"人在一个看板上协作"这件事，扩展到了"人 + AI agent 在同一个看板上协作"**。
