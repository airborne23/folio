...
5. **服务于产品定位**：Folio 的核心差异化是 **"BYO-agent 的 Linear—— agent 跑在你自己的机器，你掌控计算和 provider 选择"**。每篇的语气和深度都应该为这个叙事服务。
...
---
title: Issues
description: Issue 是 Folio 的核心工作对象——人和 agent 都能被分配、评论、改状态。
---
...
Issue 是 Folio 的核心工作对象……（1-2 段话说清楚"是什么"）

## 关键概念

### Polymorphic Assignee

Issue 的 assignee 可以是 member（人）或 agent。这是 Folio 和传统 task manager
...
```bash
folio issue assign <issue-id> --agent <agent-slug>
```

分配给 agent 时，Folio 会立刻入队一个 task 到对应 runtime。详见
...
- **目标读者**: P0 新用户 / evaluator（第一次听说 Folio）
...
### 1.2 How Folio Works — ⬜ Not started [v1]

- **Source files**: `server/cmd/folio-daemon/`, `packages/core/`, 战略 plan 的"产品定位"段
...
- **Source files**: `apps/docs/content/docs/cloud-quickstart.mdx`（现有）, `server/cmd/folio/cmd_setup.go`, `cmd_login.go`, `cmd_agent.go`, `cmd_issue.go`
- **目标读者**: P0 新用户（想 5 分钟跑起来）
- **叙事位置**: 第三页。
- **写什么**（600-1000 字）:
  - Signup → install CLI → `folio login` → `folio setup cloud` → 创建第一个 agent → 创建第一个 issue → 分配给 agent → 看它工作
  - 每步命令可独立复制运行
...
- **Source files**: `Makefile`（selfhost target）, `docker-compose.selfhost.yml`, `.env.example`, `server/cmd/folio/cmd_setup.go`
...
- **Source files**: `server/internal/handler/agent.go`, `server/pkg/db/queries/agent.sql`, `packages/views/agents/`, `server/cmd/folio/cmd_agent.go`
- **目标读者**: P1 团队管理员
- **叙事位置**: 怎么创建一个 agent。
- **写什么**（1200-1800 字）:
...
- **Source files**: `server/internal/handler/skill.go`, `server/pkg/db/queries/skill.sql`, `server/internal/daemon/execenv/context.go`（各 provider skill 注入路径）, `server/cmd/folio/cmd_skill.go`
...
- **写什么**（1200-1800 字）:
  ...
  - **兼容性宣示**：
    > "Folio Skill 采用 [Anthropic Agent Skills 开放标准](https://agentskills.io) 的 `SKILL.md` 格式。所有符合该规范的 skill（包括 Anthropic 官方仓库、ClawHub、skills.sh 上发布的包）都可以直接导入使用。"
  ...
  - **末尾一段"Skills vs MCP"**（v2 再开 MCP 独立页）:
    > MCP（Model Context Protocol）是另一层概念——让 agent 连外部工具（数据库、文件系统、第三方 API）。Folio 支持 `mcp_config` 字段，但目前**仅 Claude Code 真实消费**，其他 provider 接收但未传递。详见 v2 的 MCP 专页（开发中）。
...
> **板块叙事**：agent 怎么真的动起来——分布式执行、用户自己跑 daemon。这是 Folio 结构上区别于 Linear/Jira 的关键部分。
...
- **Source files**:
  - Daemon: `server/internal/daemon/daemon.go`, `server/cmd/folio-daemon/main.go`, `server/cmd/folio/cmd_daemon.go`, `server/pkg/db/queries/daemon.sql`
...
- **写什么**（1500-2000 字）:
  - **Daemon 部分**:
    ...
    - CLI：`folio daemon install/login/start/stop/status/logs`
  - **Runtime 部分**:
...
- **写什么**（1500-2500 字）:
  ...
  - 每个 provider 一小段（80-150 字）：核心定位 + 用户画像 + 官网链接 + Folio 兼容性
  - **Session resume 精确现状**:
...
- **写什么**（600-1000 字）:
  ...
  - CLI：`folio issue assign <id> --agent <agent-slug>`
  - 分配后立刻入队 task
...
- **写什么**（1500-2000 字）:
  - **🚨 超醒目 warning block**：生产环境必须保持 `FOLIO_DEV_VERIFICATION_CODE` 为空；固定测试验证码只用于非 production 私有测试
...
- **写什么**（1200-2000 字，v1 先覆盖 Top 6-8 问题）:
  ...
  - 固定测试验证码不工作（APP_ENV / FOLIO_DEV_VERIFICATION_CODE 检查）
  - Port 冲突
...
- **Source files**: `server/cmd/folio/main.go`（命令树）, 各 `cmd_*.go`
...
- **Source files**: `server/internal/handler/auth.go`, `server/internal/middleware/auth.go`, `server/internal/middleware/daemon_auth.go`, `server/cmd/folio/cmd_auth.go`
...
- **写什么**（1200-1800 字）:
  ...
  - PAT 创建 / 撤销 / 管理（UI 在 Settings，CLI 通过 `folio login`）
  - Daemon token 生成时机（`folio daemon login`）
...
- **写什么**（1000-1500 字）:
  ...
  - **Daemon 不内置**：desktop 只是窗口，daemon 要单独 `folio daemon start`，desktop package 里 bundle 了 CLI
...
- **板块 8**（CLI 详细 reference，从 8.1 cheatsheet 拆开）:
  - 8.4 folio auth / login 详细
  - 8.5 folio setup 详细
  - 8.6 folio workspace 详细
  - 8.7 folio issue（+comment / subscriber）详细
  - 8.8 folio project 详细
  - 8.9 folio agent（+skills）详细
  - 8.10 folio skill 详细
  - 8.11 folio autopilot 详细
  - 8.12 folio repo 详细
  - 8.13 folio daemon 详细
  - 8.14 folio runtime 详细
  - 8.15 folio config / version / update / attachment 详细
