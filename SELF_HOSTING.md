...
Deploy Folio on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`folio` CLI** and runs the **agent daemon** on their own machine.
...
This installs the `folio` CLI, checks out the latest self-host assets, pulls the official Folio images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `folio-backend:dev` / `folio-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `FOLIO_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `FOLIO_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
```bash
curl -fsSL https://raw.githubusercontent.com/folio-ai/folio/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Folio Cloud

If you've been self-hosting and want to switch your CLI to [Folio Cloud](https://folio.ai):

```bash
folio setup
```

This reconfigures the CLI for folio.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `FOLIO_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `folio setup`:
