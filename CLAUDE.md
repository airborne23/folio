...
Folio is an AI-native task management platform — like Linear, but with AI agents as first-class citizens.
...
make cli ARGS="..."   # Run folio CLI (e.g. make cli ARGS="config")
make test             # Go tests
...
pnpm --filter @folio/views exec vitest run auth/login-page.test.tsx
pnpm --filter @folio/core exec vitest run runtimes/version.test.ts
pnpm --filter @folio/web exec vitest run app/\(auth\)/login/page.test.tsx
...
pnpm --filter @folio/desktop build      # Compile TS → JS (reads .env.production)
pnpm --filter @folio/desktop package    # Package into .app/.dmg/.exe (current platform only)
...
- `packages/ui/` — zero `@folio/core` imports (pure UI, no business logic).
...
Every full-window desktop view (anything outside the dashboard shell) must mount `<DragStrip />` from `@folio/views/platform` as the first flex child of the page root, otherwise users can't drag the window. Interactive UI inside the top 48px needs `WebkitAppRegion: "no-drag"` to stay clickable.
...
**Never test shared component behavior in an app's test file.** If a test requires mocking `next/navigation` or `react-router-dom` to test a component from `@folio/views`, the test is in the wrong place — move it to `packages/views/` and mock `@folio/core` instead.
...
- Mock `@folio/core` stores with `vi.hoisted()` + `Object.assign(selectorFn, { getState })` pattern (Zustand stores are both callable and have `.getState()`).
- Mock `@folio/core/api` for API calls.
