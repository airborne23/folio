...
import { matchLocale, LOCALE_COOKIE } from "@folio/core/i18n";
...
// Forward the resolved locale to RSC layouts via the `x-folio-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set("x-folio-locale", resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("folio_logged_in");
  ...
  // --- Root path: there is no landing page ---
  // The marketing (landing) route group was deleted — `/` no longer
  // resolves to anything. Redirect every visit:
  //   - logged-in + lastSlug known → straight to that workspace
  //   - logged-in, no slug yet → /login (resolver picks workspace)
  //   - logged-out → /login (the only entry surface)
  if (pathname === "/") {
    const url = req.nextUrl.clone();
