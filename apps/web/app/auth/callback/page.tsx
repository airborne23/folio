...
import { paths, resolvePostAuthDestination } from "@folio/core/paths";
...
import {
  ...
  CardDescription,
  CardContent,
} from "@folio/ui/components/ui/card";
    ...
    if (isDesktop) {
      ...
      api
        ...
        .then(({ token }) => {
          ...
          window.location.href = `folio://auth/callback?token=${encodeURIComponent(token)}`;
        })
        .catch((err) => {
      ...
      loginWithGoogle(code, redirectUri)
        .then(async (loggedInUser) => {
          ...
          //    login on app.folio.ai instead of clicking the email link,
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">Opening Folio</CardTitle>
            <CardDescription>
              You should see a prompt to open the Folio desktop app. If
          ...
          <CardContent className="flex justify-center">
            <Button
              variant="outline"
              onClick={() => {
                window.location.href = `folio://auth/callback?token=${encodeURIComponent(desktopToken)}`;
              }}
            >
              Open Folio Desktop
            </Button>
