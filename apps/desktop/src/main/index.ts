...
// Folio CLI can find agent binaries like claude/codex/opencode. Must run
...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,
      webSecurity: false,
      additionalArguments: [`--folio-locale=${systemLocale}`],
    },
  });
...
// single-worktree case still lands at "Folio Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Folio Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Folio Canary";
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.folio.desktop.dev" : "ai.folio.desktop",
    );
