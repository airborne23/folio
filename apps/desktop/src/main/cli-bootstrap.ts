...
// Desktop prefers the bundled `folio` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/airborne23/folio/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "folio.exe" : "folio";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `folio-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `folio` binary. If one is already present at
