...
const PREFS_PATH = join(homedir(), ".folio", "desktop_prefs.json");
...
// Keep the Go impl in sync: server/cmd/folio/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".folio", "profiles", profile)
    : join(homedir(), ".folio");
}
...
// Sidecar file that records which Folio user the cached PAT in config.json
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["folio.exe"] : ["folio"];
 ...
 *   to `apps/desktop/resources/bin/folio`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Folio.app>/Contents/Resources/app.asar`.
...
/**
 * Returns a usable `folio` binary path. Priority:
 ...
 *   5. `folio` on PATH (dev convenience / user-installed via brew).
 ...
 * daemon needs a PAT (or `fol_` / `fdt_` token) because JWTs expire in 30
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Folio Desktop" }),
  });
  if (!res.ok) {
  ...
  if (typeof data.token !== "string" || !data.token.startsWith("fol_")) {
 ...
 * - If the profile already has a cached PAT (`fol_...`) AND the sidecar user
...
async function syncToken(
  ...
  const sameUserWithCachedPat =
    ...
    config.token.startsWith("fol_");

  let finalToken: string;
  if (tokenFromRenderer.startsWith("fol_")) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, FOLIO_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "Folio CLI is not installed" };
