const RELEASE_ARCHIVE_PREFIX = "folio-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `folio-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `folio_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `folio_${os}_${mappedArch}.${ext}`;
