...
import { CoreProvider } from "@folio/core/platform";
import { pickLocale } from "@folio/core/i18n";
import { useAuthStore } from "@folio/core/auth";
import { workspaceKeys, workspaceListOptions } from "@folio/core/workspace/queries";
import { api } from "@folio/core/api";
import { useHasOnboarded } from "@folio/core/paths";
import { ThemeProvider } from "@folio/ui/components/common/theme-provider";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.folio config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (folio://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (folio://auth/callback?token=...).
  ...
  if (isLoading || bootstrapping) {
    return (
      <div className="flex h-screen items-center justify-center">
        <FolioIcon className="size-6 animate-pulse" />
      </div>
    );
  }
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Folio Desktop could not load <code>~/.folio/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
