...
import { ModalRegistry } from "@folio/views/modals/registry";
import { AppSidebar } from "@folio/views/layout";
import { SearchCommand, SearchTrigger } from "@folio/views/search";
import { StarterContentPrompt } from "@folio/views/onboarding";
import { WorkspaceSlugProvider, paths, useCurrentWorkspace } from "@folio/core/paths";
import { getCurrentSlug, subscribeToCurrentSlug } from "@folio/core/platform";
import { useDesktopUnreadBadge } from "@folio/views/platform";
...
function useInternalLinkHandler() {
  useEffect(() => {
    ...
    window.addEventListener("folio:navigate", handler);
    return () => window.removeEventListener("folio:navigate", handler);
  }, []);
}
  ...
  useEffect(() => {
    return window.desktopAPI.onInboxOpen(({ slug, issueKey }) => {
      ...
      window.dispatchEvent(
        new CustomEvent("folio:navigate", { detail: { path: inboxPath } }),
      );
    });
  }, []);
