...
description: Run Folio on your own server or machine with Docker. Takes about 10 minutes.
---

import { Callout } from "fumadocs-ui/components/callout";

This page walks you through running the Folio **server** (backend + frontend + PostgreSQL) on your own machine or server with Docker. When you're done, your data is fully under your control — including [workspaces](/workspaces), [issues](/issues), [comments](/comments), and [agent](/agents) configuration.
...
2. Pull the official Docker images (PostgreSQL, Folio backend, Folio frontend)
...
**`docker-compose.selfhost.yml` sets `APP_ENV` to `production` by default** and leaves `FOLIO_DEV_VERIFICATION_CODE` empty, so there is no fixed code on public instances.

Only set `FOLIO_DEV_VERIFICATION_CODE` for local or private test automation. If a fixed code is enabled while `APP_ENV` is non-production, anyone who can request a code can sign in with that fixed value. See [Auth setup → Fixed local testing codes](/auth-setup#fixed-local-testing-codes).

Before any public deployment, make sure `.env` has `APP_ENV=production` and `FOLIO_DEV_VERIFICATION_CODE` is empty.
</Callout>
...
- Do not use `888888` unless you explicitly set `FOLIO_DEV_VERIFICATION_CODE=888888` on a non-production private instance
- Log in and create your first workspace

## 5. Point the CLI at your own server

The CLI install is the same as in [Cloud quickstart → 2. Install the CLI](/cloud-quickstart#2-install-the-folio-cli) — Homebrew / script / PowerShell, pick one. Once installed, **use the self-host variant of the setup command**:

```bash
folio setup self-host --server-url http://<your-server-address>:8080 --app-url http://<your-server-address>:3000
```
...
- [Desktop app](/desktop-app) — optional Desktop setup via `~/.folio/desktop.json`; the web frontend + CLI remains the quickest self-host path
