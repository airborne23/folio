...
Folio 里的**项目**（project）是相关 [issue](/issues) 的容器。当一摊工作比单个 issue 大、又比整个工作区小的时候用它 —— 一次发布、一次迁移、一个分多块做的功能、一个会拆出多个线索的调研。
...
每个项目有一个 **Resources** 区，可以挂 GitHub 仓库。挂上之后，被分配到这个项目里 issue 的 [智能体](/agents) 在执行 task 时可以读写这些仓库 —— Folio 会把仓库 URL 作为上下文传给 [守护进程](/daemon-runtimes)。
...
- [Folio 怎么运转](/how-folio-works) —— 整体视图
