...
description: Folio 的核心工作单位——可以分配给人，也可以分配给智能体。
---

import { Callout } from "fumadocs-ui/components/callout";

Issue（工作项）是 Folio 里一个独立工作的单位——一条 bug、一个新功能、一项要做的事。每个 issue 有**标题**、**描述**（支持 Markdown）、**状态**、**优先级**、**分配人（assignee）**，可选地还能归入某个 **project**。如果你用过 Linear 或 Jira，它们是同类东西。

**Folio 最大的特色是：issue 的分配人可以是人，也可以是 [智能体](/agents)**——这是下面先讲的第一件事。
...
Folio 提供七种状态。**任何状态可以直接改到任何状态**——Folio 不强加工作流，不会因为你从 `backlog` 直接跳到 `done` 就拦你。
