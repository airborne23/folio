...
这一页带你走一遍 Folio Cloud 的端到端流程——**注册 → 装 [命令行工具](/cli) → 启动 [守护进程](/daemon-runtimes) → 创建 [智能体](/agents) → 分配第一个 [任务](/tasks)**，约 5 分钟完成。
...
到 [folio.ai](https://folio.ai) 注册账号。可以用邮箱（6 位验证码）或 Google 登录。
...
1. 把命令行工具配置成连接 Folio Cloud
2. 打开浏览器让你登录（和 Web 登录一样的邮箱验证码 / Google OAuth）
3. 把生成的 PAT 存到 `~/.folio/config.json`
4. **自动启动守护进程**——开始每 3 秒轮询任务、每 15 秒发心跳

<Callout type="info">
**用的是桌面应用？** 桌面应用启动时**自动拉起守护进程**，不需要手动跑 `folio setup`。见 [桌面应用](/desktop-app)。
</Callout>
...
```bash
folio issue create --title "给 README 加一段 ASCII 架构图"
```
...
`--to` 后面填智能体或成员的**名字**，子串就行——如果智能体叫 `my-code-reviewer`，填 `reviewer` 也能命中。如果工作区里名字相互重叠或冲突，改用 `--to-id <uuid>`（与 `--to` 互斥）；UUID 来自 `folio agent list --output json` 或 `folio workspace members --output json`。
...
4. 结束后把结果发回 Folio（状态变 `completed` 或 `failed`，根据是否自动重试）
