...
Same as mentioning a member — type `@` to open the picker and select an agent. Once the comment is posted, Folio immediately enqueues a `task` for each mentioned agent with **that comment** as its trigger context. When the agent receives the task it can read:
...
Agents can post comments while executing, and those comments may contain `@mention`s. Folio has a hardcoded guard: **if the comment author is the same as the agent targeted by an `@` mention, that mention is skipped** — there is no "agent A @ agent A → new task → @ agent A again" infinite loop.
