...
description: The top 7 common issues when self-hosting Folio — symptoms, causes, how to diagnose, how to fix.
---

import { Callout } from "fumadocs-ui/components/callout";

Look up issues by symptom. Each entry gives you **symptom / likely causes / how to diagnose / how to fix**. If your situation isn't listed, open an issue on [GitHub](https://github.com/airborne23/folio/issues).

## Daemon can't connect to the server

**Symptom**: [`folio daemon`](/cli)'s `status` command shows `offline` or `connection refused`; the server logs show no `/api/daemon/register` or `/api/daemon/heartbeat` requests. For how the daemon mechanism works, see [Daemon and runtimes](/daemon-runtimes).

**Likely causes**:

1. **`FOLIO_SERVER_URL` points at the wrong address** — default is `ws://localhost:8080/ws`; self-host must change it to your server address
2. **Network / firewall blocking** — the daemon and server aren't on the same network, or outbound traffic is blocked
3. **Token expired or invalid** — you never ran `folio login`, or the PAT was revoked
...
```bash
folio daemon logs --lines 100    # look for daemon-side errors
echo $FOLIO_SERVER_URL          # confirm the address is set
...
cat ~/.folio/config.json        # verify api_token exists
folio workspace list            # confirm you're a member of the target workspace
```

**How to fix**: address each cause above. The two most common fixes are **changing `FOLIO_SERVER_URL` and restarting the daemon** (`folio daemon restart`) and **signing in again** (`folio logout && folio login`).

## Tasks stuck in `queued`

**Symptom**: after assigning an issue to an agent, the issue status flips to `in_progress` immediately, but a long time passes with no sign of agent execution on the page; `folio daemon status` shows the daemon `online`.
...
```bash
folio daemon status --output json       # runtime list + last_seen_at
folio agent list                         # check agent archived state
folio issue show <issue-id>             # inspect task history
```
...
- Concurrency full → wait for running tasks to finish, or `folio agent update <id> --max-concurrent-tasks 10` to raise the ceiling
- Same-issue serialization → wait for the previous task to finish, or reassign to a different agent
- Agent archived → `folio agent restore <id>`
- Runtime not registered → `folio daemon restart`, and the daemon will re-register
...
- Wrong origin → set `FRONTEND_ORIGIN=https://folio.yourdomain.com` in the server's `.env` (or comma-separated `CORS_ALLOWED_ORIGINS`) and restart the server
...
1. **`FOLIO_DEV_VERIFICATION_CODE` is empty** — fixed codes are disabled by default
...
```bash
cat .env | grep -E 'APP_ENV|FOLIO_DEV_VERIFICATION_CODE'
docker exec <container> env | grep -E 'APP_ENV|FOLIO_DEV_VERIFICATION_CODE'
```
...
|---|---|---|
| **Daemon** | `~/.folio/daemon.log` (background mode) or foreground stdout | `folio daemon logs -f --lines 100` |
| **Server (Docker)** | Container stdout | `docker logs -f <container>` |
| **Server (systemd)** | journal | `journalctl -u folio-server -f` |
| **Frontend (dev)** | Terminal running `pnpm dev` | Read directly |
| **Frontend (browser)** | DevTools → Console | Press `F12` |

For more detailed daemon logs, move it from background to foreground: `folio daemon stop && folio daemon start --foreground`.
