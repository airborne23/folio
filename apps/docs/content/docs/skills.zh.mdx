...
Skill 是给 [智能体](/agents) 的**专业知识包**——一个 `SKILL.md` 加上可选的支持文件（脚本、配置、参考模板等），告诉智能体"遇到某类任务时该怎么想、怎么做"。Folio 采用 [Anthropic Agent Skills](https://agentskills.io) 开放标准，所有符合规范的 Skill（Anthropic 官方仓库、ClawHub、skills.sh 上的包）都能直接导入。

## 工作区 Skill 和本机 Skill

Folio 支持两种 Skill 来源：

- **工作区 Skill（workspace skill）** —— 存在 Folio 云端。挂到智能体后，任务执行时自动同步到你本机的守护进程。这是**团队共享 Skill 的标准方式**。
...
- **从 GitHub** —— 贴一个仓库 URL（例如 `https://github.com/owner/repo/tree/main/skills/my-skill`），Folio 自动拉取目录下的 `SKILL.md` 和所有文件
...
两者可以同时用。目前 Folio 的 MCP 支持**只有 Claude Code 真正消费**——其他工具会接收到 MCP 配置但不会实际用。MCP 的专题会在后续版本展开。
