...
Folio supports two sign-in methods: **email + verification code** (default) and **Google OAuth** (optional). On successful sign-in, the server issues a JWT cookie with a 30-day lifetime. This page covers how to configure each method, how to restrict who can sign up, and the single biggest trap for self-hosted deployments.
...
Local development without Resend should use the generated code printed in server logs. If you need deterministic local/private automation, set `FOLIO_DEV_VERIFICATION_CODE` to a 6-digit value such as `888888`, and keep `APP_ENV` non-production:
...
Production deployments should leave `FOLIO_DEV_VERIFICATION_CODE` empty and set `APP_ENV=production`. If you deploy via `make selfhost` / `docker-compose.selfhost.yml`, `APP_ENV` defaults to `production`.
...
2. Set the **Authorized redirect URIs** to your Folio frontend address plus `/auth/callback`, for example:

    ```text
    https://folio.yourdomain.com/auth/callback
    ```
...
3. Once you have the client ID and client secret, set three environment variables:
    ...
    GOOGLE_REDIRECT_URI=https://folio.yourdomain.com/auth/callback
    ```
...
**Only people who already have a Folio account.** Accepting an invite doesn't check the signup allowlist — if the invitee has signed up already (for example in another workspace), clicking the invite link and signing in lets them accept.
