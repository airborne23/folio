...
运行时背后是一款具体的 AI 编程工具。Folio 支持 11 款，最常用的几款：
...
系统关键变量不能被覆盖：`PATH`、`HOME`、`USER`、`SHELL`、`TERM`、`CODEX_HOME`，以及任何 `FOLIO_*` 开头的 key 都会被守护进程静默忽略（日志里有 warn，不会报错）。

<Callout type="warning">
**`custom_env` 的值在 Folio 服务器的数据库里是明文存储的。** 非智能体创建者 / 非 workspace admin 看不到值（API 返回 `****`），但数据库备份、DB 审计里仍然能看到。
