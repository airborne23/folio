---
title: AI 编程工具对照
description: Folio 支持 11 款 AI 编程工具；它们实现同一套接口，但能力细节差异很大。
---

import { Callout } from "fumadocs-ui/components/callout";

Folio 内置支持 **11 款 AI 编程工具**。它们都实现了同一套接口——排队、派发、执行、结果回传，所以你可以从 Folio 的同一个看板上指挥任意一款。**但它们在能力细节上差异很大**：会话恢复是否真用、是否支持 MCP、skill 文件该放在哪里、模型怎么选。这一页是完整对照。
...
每款工具用**自己**的 skill 发现路径。Folio 的守护进程在执行任务前把 workspace 的 skill 文件复制到对应路径下：
...
- [守护进程与运行时](/daemon-runtimes) —— 工具跑在哪里、怎么连进 Folio
