...
description: 在自己的服务器或本机用 Docker 把 Folio 跑起来。约 10 分钟。
---

import { Callout } from "fumadocs-ui/components/callout";

这一页带你用 Docker 把 Folio 的**服务器**（后端 + 前端 + PostgreSQL）跑在自己的机器或服务器上。走完这一篇你的数据就完全在自己手里——包括 [工作区](/workspaces)、[issue](/issues)、[评论](/comments)、[智能体](/agents) 配置。
...
2. 拉取官方 Docker 镜像（PostgreSQL、Folio backend、Folio frontend）
...
**`docker-compose.selfhost.yml` 默认把 `APP_ENV` 设成 `production`**，并让 `FOLIO_DEV_VERIFICATION_CODE` 为空，所以公网实例默认没有固定验证码。

只在本地或私有测试自动化里设置 `FOLIO_DEV_VERIFICATION_CODE`。如果在 `APP_ENV` 非 production 时启用了固定验证码，任何能请求验证码的人都能用这个固定值登录。详见 [登录与注册配置 → 固定本地测试验证码](/auth-setup#固定本地测试验证码)。

公网部署前一定检查 `.env` 里 `APP_ENV=production`，且 `FOLIO_DEV_VERIFICATION_CODE` 为空。
</Callout>
...
- 不要直接使用 `888888`；只有在非 production 私有实例上显式设置 `FOLIO_DEV_VERIFICATION_CODE=888888` 后它才会生效
- 登录后创建第一个工作区

## 5. 连接命令行工具到你自己的 server

命令行装法和 [Cloud 快速上手 → 2. 装命令行工具](/cloud-quickstart#2-装-folio-命令行工具) 一样——Homebrew / 脚本 / PowerShell 任选。装好之后，**用 self-host 版本的 setup 命令**：

```bash
folio setup self-host --server-url http://<你的服务器地址>:8080 --app-url http://<你的服务器地址>:3000
```
...
- [桌面应用](/desktop-app) —— 可以通过 `~/.folio/desktop.json` 连接 Desktop；Web 前端 + CLI 仍然是最快的自部署路径
