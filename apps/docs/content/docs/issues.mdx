...
description: Folio's core unit of work — assignable to a person or to an agent.
---

import { Callout } from "fumadocs-ui/components/callout";

An issue is a self-contained unit of work in Folio — a bug, a new feature, a thing that needs doing. Every issue has a **title**, a **description** (Markdown supported), a **status**, a **priority**, an **assignee**, and optionally belongs to a **project**. If you've used Linear or Jira, this is the same shape.

**Folio's defining trait is that an issue's assignee can be a person or an [agent](/agents)** — which is where we'll start.
...
Folio has seven statuses. **Any status can move directly to any other** — Folio doesn't impose a workflow, and won't stop you from jumping from `backlog` straight to `done`.
