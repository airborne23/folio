...
| `packages/ui` | 无业务依赖 | `@folio/core`、业务逻辑 |
...
- **实体（typed entity）** —— 有 URL、有数据库 row、是 API 响应里某种 type 的东西。中文里**用小写英文**呈现，视觉上像类型名，告诉读者"这是 Folio 系统里的特定实体"。
...
`issue` / `skill` / `task` 是 Folio 的核心实体。schema 字段、API 字段、产品 UI 标签都用英文。中文里采用**混合规则** —— 词出现在哪里决定怎么写：
...
| 类别 | 词 |
| --- | --- |
| 品牌 | **Folio**、GitHub、Slack、Google、Anthropic、OpenAI、Claude、Codex、Cursor、Linear、Jira |
