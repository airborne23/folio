...
description: Deploy Folio on your own infrastructure.
---
...
Each user who wants to run AI agents locally also installs the **`folio` CLI** and runs the **agent daemon** on their own machine.
...
<Callout>
Alternatively, configure step by step: `folio config set server_url http://localhost:8080 && folio config set app_url http://localhost:3000 && folio login && folio daemon start`
</Callout>
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`folio`/`folio`).
