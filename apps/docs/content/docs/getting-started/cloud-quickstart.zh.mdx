...
description: Get started with Folio Cloud — no setup required.
---

The fastest way to get started with Folio — no setup required.

## 1. Sign up

Go to [folio.ai](https://folio.ai) and create an account.
...
```
Fetch https://github.com/airborne23/folio/blob/main/CLI_INSTALL.md and follow the instructions to install Folio CLI, log in, and start the daemon on this machine.
```
...
Open your workspace in the Folio web app. Navigate to **Settings → Runtimes** — you should see your machine listed as an active **Runtime**.

> **What is a Runtime?** A Runtime is a compute environment that can execute agent tasks. It can be your local machine (via the daemon) or a cloud instance. Each runtime reports which agent CLIs are available, so Folio knows where to route work.
...
Create an issue from the board (or via `folio issue create`), then assign it to your new agent. The agent will automatically pick up the task, execute it on your runtime, and report progress — just like a human teammate.
