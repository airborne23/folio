...
Folio 是一个**分布式**平台。你看到的 Web 界面只是前台——真正干活的有三个组件：**Folio 服务器**管数据（[工作区](/workspaces)、[issue](/issues)、[成员](/members-roles)、[任务](/tasks) 队列等）；**[守护进程](/daemon-runtimes)** 跑在你自己机器上，领任务、调用 AI 编程工具；**[AI 编程工具](/providers)**（Claude Code、Codex 等本地 CLI）是真正写代码的那一环。这个结构是 Folio 和 Linear / Jira 最大的差别——**[智能体](/agents) 不跑在我们的服务器上，而是在你自己的机器上**。

## 系统的三个核心组件

<ArchitectureDiagram />

- **Folio 服务器**——你看到的工作区、issue 列表、评论线都存在它的数据库里。它同时是 WebSocket hub，把你和同事之间的实时更新推送过去。它**不**执行任何智能体任务。
- **守护进程**（daemon）——Folio CLI 的一部分，跑在你自己的机器上。启动后它探测本地装了哪些 AI 编程工具，注册到 server，开始每 3 秒领一次任务、每 15 秒发一次心跳。
...
工具链在本地的结果：**你的 API 密钥、代码目录、已授权的工具**都只在本地使用；Folio 服务器一个都看不到。自部署还是用 Cloud 都不改变这一点。

## 一个任务从创建到完成会经历什么

以"你把一个 issue 分配给某个智能体"这个最常见的场景为例：

1. 你在 Web 上点击分配。浏览器发 HTTP 请求到 Folio 服务器。
...
不只是"分配 issue"——Folio 有 4 种触发方式，对应不同协作场景：
...
**运行时**（runtime）是"守护进程 × 一款 AI 编程工具"的组合。同一台机器上的守护进程装了 Claude Code 和 Codex，两个工作区都加入了，那么 Folio 会注册 4 个独立运行时（2 工作区 × 2 工具）。

目前只支持**本地守护进程**这一种运行模式。云端运行时（不需要你自己开机）**即将开放**，当前处于等待名单阶段——在 [下载页面](https://folio.ai/download) 登记邮箱。

## 下一步

- [Cloud Quickstart](/cloud-quickstart) —— 5 分钟接入 Folio Cloud
