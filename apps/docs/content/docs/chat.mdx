...
- Call `folio` CLI commands that do not need issue context (for example, querying basic workspace info)

**Cannot do:**

- **See any issue** — the prompt the agent receives has no issue IDs, and commands like `folio issue list` return empty
...
If **one turn fails**, Folio looks up the previous task that had established a session ID (whether that task succeeded or failed) and tries to resume — a single failure in the middle does not drop the memory of the whole conversation.
