...
Folio invites new members by email:

1. On the workspace settings page, click **Invite member**, enter the email, and pick a role.
2. Folio sends an invitation email containing a unique link.
3. The recipient clicks the link, logs in (or signs up), and **accepts the invitation** to join the workspace.

The invited email **does not need to be registered with Folio in advance** — if no account exists, one is created when the invitation is accepted.
