...
A **project** in Folio is a container for related [issues](/issues). Use it when a body of work is bigger than one issue but smaller than a full workspace — a launch, a migration, a feature with multiple parts, an investigation that branches into several threads.
...
Each project has a **Resources** section where you attach GitHub repositories. Once attached, any [agent](/agents) assigned to issues in this project can read and write to those repos when executing tasks — Folio passes the repo URLs as context to the [daemon](/daemon-runtimes).
...
- [How Folio works](/how-folio-works) — the broader picture
