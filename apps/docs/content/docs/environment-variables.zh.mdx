---
title: 环境变量
description: self-host Folio 服务器需要配置的环境变量清单。
---

import { Callout } from "fumadocs-ui/components/callout";

Folio 的 [自部署](/self-host-quickstart) 服务器启动时从环境变量读取配置——数据库、登录、邮件、存储、注册白名单都在这里配。这一页按用途分组给完整清单：每组说清楚**不设会怎样**、**生产必须设哪几个**。Auth 相关那几个怎么真正配见 [登录与注册配置](/auth-setup)。
...
| 环境变量 | 默认值 | 生产必须设？ |
|---|---|---|
| `DATABASE_URL` | `postgres://folio:folio@localhost:5432/folio?sslmode=disable` | **是** |
| `PORT` | `8080` | 否（除非换端口）|
| `JWT_SECRET` | `folio-dev-secret-change-in-production` | **是**（默认值不安全）|
| `APP_ENV` | 空 | **是**（必须 `production`）|
| `FRONTEND_ORIGIN` | 空 | **是**（self-host 要填你自己的域名）|
| `FOLIO_DEV_VERIFICATION_CODE` | 空 | 否（生产必须保持为空）|

<Callout type="warning">
**生产环境保持 `FOLIO_DEV_VERIFICATION_CODE` 为空。** 固定本地测试验证码默认关闭；如果你设置 `FOLIO_DEV_VERIFICATION_CODE=888888`，在 `APP_ENV` 非 production 时，任何能请求验证码的人都能用这个固定值登录。`APP_ENV=production` 时该快捷码会被忽略。
</Callout>
...
Folio 用 [Resend](https://resend.com/) 发验证码和邀请邮件。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `RESEND_API_KEY` | 空 | Resend API key |
| `RESEND_FROM_EMAIL` | `noreply@folio.ai` | 发件地址（必须是 Resend 账号已验证的域名）|
...
Folio 存储用户上传的附件（评论里的图片、文件等）。**优先走 S3**；不配 S3 就回落本地磁盘。
...
**邀请流程本身不检查 signup 白名单**——但被邀请人必须先能**登录**才能接受邀请。如果对方已经有 Folio 账号（比如在其他工作区注册过），可以直接接受，不受白名单影响；**如果对方还没注册过**，他们登录的第一步（发送验证码）仍然会过白名单检查，被 `ALLOW_SIGNUP=false` 或 `ALLOWED_EMAILS` / `ALLOWED_EMAIL_DOMAINS` 拒绝的邮箱**无法完成注册，也就没法接受邀请**。
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `FOLIO_SERVER_URL` | `ws://localhost:8080/ws` | server 地址（self-host 换成你的域名）|
| `FOLIO_DAEMON_HEARTBEAT_INTERVAL` | `15s` | 心跳频率 |
| `FOLIO_DAEMON_POLL_INTERVAL` | `3s` | 任务轮询频率 |
| `FOLIO_DAEMON_MAX_CONCURRENT_TASKS` | `20` | 并发任务上限 |
| `FOLIO_<PROVIDER>_PATH` | 对应 CLI 名 | 各 AI 编程工具的可执行文件路径（如 `FOLIO_CLAUDE_PATH`）|
| `FOLIO_<PROVIDER>_MODEL` | 空 | 各 AI 编程工具的默认模型 |
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `FRONTEND_ORIGIN` | 空 | 前端地址。邀请邮件里的链接、CORS 白名单、cookie domain 都从这里推导。邮件链接在不设时会 fallback 到托管版域名 `https://app.folio.ai`——self-host 必须显式填 |
...
<Callout type="warning">
**`FRONTEND_ORIGIN` 不设就有两个静默失败**：（1）邀请邮件里的链接指向 `https://app.folio.ai`（托管版的域名），用户点了跳不回你的 self-host 实例；（2）WebSocket 连接的 Origin 校验回落到 `localhost:3000 / 5173 / 5174`，生产部署的 WebSocket 全部被拒，前端看起来「实时更新不工作」。
</Callout>

## 用量统计

默认上报到 Folio 官方 PostHog 实例。不想上报就把 `ANALYTICS_DISABLED=true`。
...
- [守护进程与运行时](/daemon-runtimes) —— `FOLIO_DAEMON_*` 参数的行为含义
