...
This document covers advanced configuration for self-hosted Folio deployments. For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgres://folio:folio@localhost:5432/folio?sslmode=disable` |
...
Folio uses email-based magic link authentication via [Resend](https://resend.com).

| Variable | Description |
|----------|-------------|
| `RESEND_API_KEY` | Your Resend API key |
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@folio.ai`) |

> **Note:** If Resend is not configured, generated verification codes are printed to backend logs. A fixed local testing code is disabled by default; to opt in on a private test instance, set `APP_ENV=development` and `FOLIO_DEV_VERIFICATION_CODE` to a 6-digit value. It is ignored when `APP_ENV=production`.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `FOLIO_SERVER_URL` | `ws://localhost:8080/ws` | WebSocket URL for daemon → server connection |
| `FOLIO_APP_URL` | `http://localhost:3000` | Frontend URL for CLI login flow |
| `FOLIO_DAEMON_POLL_INTERVAL` | `3s` | How often the daemon polls for tasks |
| `FOLIO_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat frequency |
...
| `FOLIO_COPILOT_PATH` | Custom path to the `copilot` (GitHub Copilot CLI) binary |
...
Folio requires PostgreSQL 17 with the pgvector extension.
...
By default, Folio works on `localhost`. If you access it from another machine on the LAN (e.g. `http://192.168.1.100:3000`), you need to tell the backend to accept that origin:
...
Pin `FOLIO_IMAGE_TAG` in `.env` to an exact release like `v0.2.4` if you want to stay on a specific version. Migrations run automatically on backend startup. They are idempotent — running them multiple times has no effect.
