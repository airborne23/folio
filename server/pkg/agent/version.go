...
// the Folio CLI version reported by the daemon at registration time. The
...
// `folio issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("Folio CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("Folio CLI version is below required minimum")
)
