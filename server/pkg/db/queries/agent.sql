...
-- Channel tasks have no issue_id; the channel_id discriminator identifies the
-- conversation, and context JSONB carries trigger_message_id / parent_message_id
-- / provider_session_id so the daemon can resume the right thread.
INSERT INTO agent_task_queue (agent_id, runtime_id, issue_id, status, priority, channel_id, context)
