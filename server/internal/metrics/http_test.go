...
func TestHTTPMiddlewareUsesRoutePatternLabels(t *testing.T) {
	...
	for _, want := range []string{
		`folio_http_requests_total{method="GET",route="/api/issues/{id}",status="201"} 1`,
		`folio_build_info{commit="abc123",version="v-test"} 1`,
	} {
...
func TestMetricsHandlerOnlyServesMetricsPath(t *testing.T) {
	...
	if body, _ := io.ReadAll(rec.Body); !strings.Contains(string(body), "folio_build_info") {
