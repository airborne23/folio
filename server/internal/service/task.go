...
import (
	...
	"github.com/airborne23/folio/server/internal/mention"
...
// `folio issue create` call. Pre-validates that the agent is reachable
...
// EnqueueChannelTaskParams carries the per-call inputs for EnqueueChannelTask.
type EnqueueChannelTaskParams struct {
	AgentID   pgtype.UUID
...
// EnqueueChannelTask creates and enqueues an agent_task_queue row for a
// channel-context dispatch (e.g. a member @-mentioned an agent in a channel).
// Mirrors EnqueueChatTask: validates the agent is alive + has a runtime,
// inserts via CreateChannelTask, then broadcasts task:queued and notifies the
// daemon wakeup channel so the agent claim happens immediately rather than on
// the next 30s poll tick.
func (s *TaskService) EnqueueChannelTask(ctx context.Context, p EnqueueChannelTaskParams) (db.AgentTaskQueue, error) {
	agent, err := s.Queries.GetAgent(ctx, p.AgentID)
	if err != nil {
		slog.Error("channel task enqueue failed", "channel_id", util.UUIDToString(p.ChannelID), "error", err)
		return db.AgentTaskQueue{}, fmt.Errorf("load agent: %w", err)
	}
	if agent.ArchivedAt.Valid {
		return db.AgentTaskQueue{}, fmt.Errorf("agent is archived")
	...
	// Same broadcast + wakeup ordering as EnqueueChatTask. Without notifyTaskAvailable
	// the daemon would wait up to its full poll interval (default 30s) before
	// claiming the row — unacceptable for an interactive @mention reply.
	s.broadcastTaskEvent(ctx, protocol.EventTaskQueued, task)
	// Channel-only "thinking" indicator so the UI can show "<agent> is
	// replying…" while the task runs. Implicit end signal is the eventual
	// channel:message:created with the same agent_id; no separate end
	// event is published. workspace_id rides on the envelope for the
	// regular workspace fanout path.
	//
	// parent_message_id rides on the dispatcher-built Context blob (see
	// channel_dispatcher.go); we surface it on the thinking event so the
	// client can scope the indicator to the right surface (main channel
	// vs thread drawer). Failure to parse leaves it empty — main channel
	// fallback, which is the safer default.
	var parentMessageID string
	if len(p.Context) > 0 {
		var ctxObj struct {
			ParentMessageID *string `json:"parent_message_id"`
		}
		if err := json.Unmarshal(p.Context, &ctxObj); err == nil && ctxObj.ParentMessageID != nil {
			parentMessageID = *ctxObj.ParentMessageID
		}
	}
	s.Bus.Publish(events.ChannelAgentThinking(
		util.UUIDToString(agent.WorkspaceID),
		util.UUIDToString(p.ChannelID),
		util.UUIDToString(p.AgentID),
		parentMessageID,
	))
	s.notifyTaskAvailable(task)
	return task, nil
...
// manual `folio agent update <id> --status idle` to unwedge. Matches the
...
func (s *TaskService) ResolveTaskWorkspaceID(ctx context.Context, task db.AgentTaskQueue) string {
	...
	if task.ChannelID.Valid {
		if wsID, err := s.Queries.GetChannelWorkspaceID(ctx, task.ChannelID); err == nil {
...
// daemon-injected FOLIO_QUICK_CREATE_TASK_ID env var, so this lookup is
