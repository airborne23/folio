...
// same machine shares one UUID stored at `~/.folio/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.folio/profiles/*/daemon.id` and returns every
...
// explicitly pins FOLIO_DAEMON_ID to the hostname itself, there's nothing
