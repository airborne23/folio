...
func TestPatternsFromEnv_DefaultsWhenUnset(t *testing.T) {
	t.Setenv("FOLIO_GC_ARTIFACT_PATTERNS", "")
	defaults := []string{"node_modules", ".next", ".turbo"}
	got := patternsFromEnv("FOLIO_GC_ARTIFACT_PATTERNS", defaults)
...
func TestPatternsFromEnv_DropsSeparatorBearingEntries(t *testing.T) {
	t.Setenv("FOLIO_GC_ARTIFACT_PATTERNS", "node_modules, .next ,foo/bar, ../etc, ,target")
	got := patternsFromEnv("FOLIO_GC_ARTIFACT_PATTERNS", nil)
