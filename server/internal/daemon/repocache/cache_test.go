...
// for the Folio Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: folio-agent <github@folio.ai>"
...
// path: a bare cache that already carries the Folio prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: folio-agent") {
...
// prepare-commit-msg hook that does NOT include the folioHookMarker
// sentinel — only the older `# Installed by the Folio daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no folioHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: folio-agent") {
...
// without the Folio marker (e.g. one a user added manually) must be left
