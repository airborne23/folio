...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/folio_workspaces/.repos)
	logger *slog.Logger
...
// folioHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const folioHookMarker = "# folio:prepare-commit-msg:co-authored-by"
...
// treats a hook as Folio-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before folioHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	folioHookMarker,
	"# Installed by the Folio daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Folio Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# folio:prepare-commit-msg:co-authored-by
# Folio: add Co-authored-by trailer for the Folio Agent.
# Installed by the Folio daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: folio-agent <github@folio.ai>"
...
// a Co-authored-by trailer for the Folio Agent. The hook is installed in the
...
// installed by the Folio daemon (current or any previously released
