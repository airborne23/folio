...
func TestBuildPromptCommentTriggered(t *testing.T) {
	...
	for _, want := range []string{
		...
		commentID,
		"folio issue comment add " + issueID + " --parent " + commentID,
	...
	if !strings.Contains(prompt, "folio issue get") {
...
// be allowed for `folio repo checkout` after registerTaskRepos runs. Without
...
func TestShellArgsFromEnv(t *testing.T) {
	t.Setenv("FOLIO_CLAUDE_ARGS", `--max-turns 60 --append-system-prompt "multi word"`)
	got, err := shellArgsFromEnv("FOLIO_CLAUDE_ARGS")
	if err != nil {
...
func TestShellArgsFromEnvEmptyIsNil(t *testing.T) {
	t.Setenv("FOLIO_CODEX_ARGS", "   ")
	got, err := shellArgsFromEnv("FOLIO_CODEX_ARGS")
	if err != nil {
		t.Fatalf("shellArgsFromEnv: %v", err)
	}
	if got != nil {
