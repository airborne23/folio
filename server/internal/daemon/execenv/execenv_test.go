...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/airborne23/folio","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".folio", "project", "resources.json")
	...
	for _, want := range []string{
		...
		"Agent UX 2026",
		"GitHub repo",
		"https://github.com/airborne23/folio",
		"default branch: `main`",
		".folio/project/resources.json",
	} {
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".folio", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"folio autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `folio issue get") {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Folio Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "folio issue get") {
		t.Error("should reference Folio CLI even without skills")
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `folio issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"folio issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `folio issue comment add`",
...
// any multi-line content for `folio issue comment add` must go through
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, folioManagedBeginMarker) || !strings.Contains(s, folioManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertFolioManagedBlock
	// for why.
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), folioManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.folio.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, folioManagedBeginMarker)
	endIdx := strings.Index(s, folioManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.folio]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.folio]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.folio content")
	}
	...
	if n := strings.Count(string(data2), folioManagedBeginMarker); n != 1 {
...
` + folioManagedBeginMarker + `
sandbox_mode = "workspace-write"

[sandbox_workspace_write]
network_access = true
` + folioManagedEndMarker + `
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.folio], got:\n%s", s)
	}
	if strings.Count(s, folioManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), folioManagedBeginMarker) {
		t.Error("reused config.toml missing folio-managed block")
	}
}
