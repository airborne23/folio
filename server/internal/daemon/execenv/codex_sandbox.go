...
// `no such host` errors when calling out (for example, `folio issue get`
// hitting the Folio API). See upstream issue openai/codex#10390.
//
...
// actually reach the Folio API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Folio CLI can reach the API.
...
// folioManagedBeginMarker / folioManagedEndMarker delimit the block the
...
const (
	folioManagedBeginMarker = "# BEGIN folio-managed (do not edit; regenerated by daemon)"
	folioManagedEndMarker   = "# END folio-managed"
)

// renderFolioManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.folio]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertFolioManagedBlock), avoids both traps.
func renderFolioManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(folioManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(folioManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(folioManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(folioManagedEndMarker) + `\n*`)

// upsertFolioManagedBlock returns the config content with the folio-managed
...
// `[permissions.folio]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderFolioManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertFolioManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderFolioManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the folio-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertFolioManagedBlock(existing, policy)
