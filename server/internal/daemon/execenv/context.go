...
// writeProjectResources writes .folio/project/resources.json into the
...
func writeProjectResources(workDir string, ctx TaskContextForEnv) error {
	...
	dir := filepath.Join(workDir, ".folio", "project")
...
func renderIssueContext(provider string, ctx TaskContextForEnv) string {
	...
	fmt.Fprintf(&b, "Run `folio issue get %s --output json` to fetch the full issue details.\n\n", ctx.IssueID)
...
func renderAutopilotContext(ctx TaskContextForEnv) string {
	...
	b.WriteString("## Quick Start\n\n")
	b.WriteString("This is a run-only autopilot task with no assigned issue. Do not run `folio issue get` unless the autopilot instructions explicitly ask you to create or update an issue.\n\n")
	if ctx.AutopilotID != "" {
