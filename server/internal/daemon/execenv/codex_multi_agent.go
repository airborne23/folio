...
// subagents. The Folio daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Folio runtime
...
//
// Users who explicitly want Codex native subagents inside a Folio task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `FOLIO_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// FolioCodexMultiAgentEnv is the env var users can set to keep Codex
...
const FolioCodexMultiAgentEnv = "FOLIO_CODEX_MULTI_AGENT"

// folioMultiAgentBeginMarker / folioMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	folioMultiAgentBeginMarker = "# BEGIN folio-managed multi-agent (do not edit; regenerated by daemon)"
	folioMultiAgentEndMarker   = "# END folio-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(folioMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(folioMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(FolioCodexMultiAgentEnv))
...
// renderFolioMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderFolioMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(folioMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(folioMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderFolioMultiAgentBlock(true)
...
//
// When FOLIO_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per FOLIO_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderFolioMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
