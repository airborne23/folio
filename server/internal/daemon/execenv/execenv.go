...
// checked out on demand by the agent via `folio repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string            // base path for all envs (e.g., ~/folio_workspaces)
...
// on demand via `folio repo checkout <url>`.
