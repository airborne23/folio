...
func BuildPrompt(task Task) string {
	...
	// Channel-context task: server already rendered the full conversation
	// history + mention context into ChannelPrompt. There is NO assigned
	// issue, so we must NOT fall through to the issue branch below — that
	// scaffolding ("Your assigned issue ID is: ...") would prompt the agent
	// to look up an issue that doesn't exist and stall on the search.
	if task.ChannelPrompt != "" {
		return task.ChannelPrompt
	}
	var b strings.Builder
	b.WriteString("You are running as a local coding agent for a Folio workspace.\n\n")
	fmt.Fprintf(&b, "Your assigned issue ID is: %s\n\n", task.IssueID)
	fmt.Fprintf(&b, "Start by running `folio issue get %s --output json` to understand your task, then complete it.\n", task.IssueID)
	fmt.Fprintf(&b, "If you need comment history, `folio issue comment list %s` returns the latest 50 by default — pass --limit or --since to scope older windows. Long issues can have thousands of comments; do not fetch everything blindly.\n", task.IssueID)
	return b.String()
}

// buildQuickCreatePrompt constructs a prompt for quick-create tasks. The
// user typed a single natural-language sentence in the create-issue modal;
// the agent's job is to translate it into one `folio issue create` CLI
...
// call `folio issue get` or attempt to comment — there's nothing to read
// or reply to.
func buildQuickCreatePrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a quick-create assistant for a Folio workspace.\n\n")
	b.WriteString("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `folio issue create` command.\n\n")
	...
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\"), call `folio workspace members --output json` (and `folio agent list --output json` if it might be an agent) and find the matching entity by display name. On a clean unambiguous match, prefer `--assignee-id <uuid>` using the `user_id` (member) or `id` (agent) from that JSON — UUID matching is exact and robust to name collisions in workspaces with overlapping names. `--assignee <name>` (fuzzy) is acceptable as a fallback when names are unambiguous. On no match or ambiguous match, do NOT pass either flag — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	agentID := ""
	agentName := ""
	if task.Agent != nil {
	...
	b.WriteString("- Run exactly one `folio issue create` invocation. Do not retry for any reason — even on non-zero exit. The issue may already exist; another attempt would create a duplicate.\n")
	b.WriteString("- After success, print exactly one line: `Created MUL-<n>: <title>` and exit. No commentary, no follow-up tool calls.\n")
	b.WriteString("- Do NOT call `folio issue get` or `folio issue comment add` — there is no issue to query or comment on.\n")
...
func buildCommentPrompt(task Task) string {
	...
	fmt.Fprintf(&b, "Start by running `folio issue get %s --output json` to understand your task, then decide how to proceed.\n\n", task.IssueID)
	fmt.Fprintf(&b, "If you need comment history, `folio issue comment list %s` returns the latest 50 by default — pass --limit or --since to scope older windows. Long issues can have thousands of comments; do not fetch everything blindly.\n\n", task.IssueID)
...
func buildChatPrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a chat assistant for a Folio workspace.\n")
...
func buildAutopilotPrompt(task Task) string {
	...
	if task.AutopilotID != "" {
		fmt.Fprintf(&b, "Start by running `folio autopilot get %s --output json` if you need the full autopilot configuration, then complete the instructions above.\n", task.AutopilotID)
	} else {
		b.WriteString("Complete the instructions above.\n")
	}
	b.WriteString("Do not run `folio issue get`; this run does not have an issue ID.\n")
	return b.String()
}
