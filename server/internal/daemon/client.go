...
// Client handles HTTP communication with the Folio server daemon API.
type Client struct {
	baseURL string
	token   string
	client  *http.Client
...
// ReportChannelChunk pushes one streaming text token to the server, which
// appends it to the channel_message body for the channel task currently
// running. Server is idempotent on chunk replay (text is concatenated, not
// deduplicated, so callers must not re-send the same chunk twice).
func (c *Client) ReportChannelChunk(ctx context.Context, taskID, chunk string) error {
	if chunk == "" {
		return nil
	}
	return c.postJSON(ctx, fmt.Sprintf("/api/daemon/tasks/%s/progress", taskID), map[string]any{
