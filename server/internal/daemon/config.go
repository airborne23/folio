...
// FOLIO_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // Folio CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/folio_workspaces)
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("FOLIO_SERVER_URL", DefaultServerURL)
	...
	claudePath := envOrDefault("FOLIO_CLAUDE_PATH", "claude")
	if _, err := exec.LookPath(claudePath); err == nil {
		agents["claude"] = AgentEntry{
			Path:  claudePath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_CLAUDE_MODEL")),
		}
	}
	codexPath := envOrDefault("FOLIO_CODEX_PATH", "codex")
	if _, err := exec.LookPath(codexPath); err == nil {
		agents["codex"] = AgentEntry{
			Path:  codexPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_CODEX_MODEL")),
		}
	}
	opencodePath := envOrDefault("FOLIO_OPENCODE_PATH", "opencode")
	if _, err := exec.LookPath(opencodePath); err == nil {
		agents["opencode"] = AgentEntry{
			Path:  opencodePath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_OPENCODE_MODEL")),
		}
	}
	openclawPath := envOrDefault("FOLIO_OPENCLAW_PATH", "openclaw")
	if _, err := exec.LookPath(openclawPath); err == nil {
		agents["openclaw"] = AgentEntry{
			Path:  openclawPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_OPENCLAW_MODEL")),
		}
	}
	hermesPath := envOrDefault("FOLIO_HERMES_PATH", "hermes")
	if _, err := exec.LookPath(hermesPath); err == nil {
		agents["hermes"] = AgentEntry{
			Path:  hermesPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_HERMES_MODEL")),
		}
	}
	geminiPath := envOrDefault("FOLIO_GEMINI_PATH", "gemini")
	if _, err := exec.LookPath(geminiPath); err == nil {
		agents["gemini"] = AgentEntry{
			Path:  geminiPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_GEMINI_MODEL")),
		}
	}
	piPath := envOrDefault("FOLIO_PI_PATH", "pi")
	if _, err := exec.LookPath(piPath); err == nil {
		agents["pi"] = AgentEntry{
			Path:  piPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_PI_MODEL")),
		}
	}
	cursorPath := envOrDefault("FOLIO_CURSOR_PATH", "cursor-agent")
	if _, err := exec.LookPath(cursorPath); err == nil {
		agents["cursor"] = AgentEntry{
			Path:  cursorPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_CURSOR_MODEL")),
		}
	}
	copilotPath := envOrDefault("FOLIO_COPILOT_PATH", "copilot")
	if _, err := exec.LookPath(copilotPath); err == nil {
		agents["copilot"] = AgentEntry{
			Path:  copilotPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_COPILOT_MODEL")),
		}
	}
	kimiPath := envOrDefault("FOLIO_KIMI_PATH", "kimi")
	if _, err := exec.LookPath(kimiPath); err == nil {
		agents["kimi"] = AgentEntry{
			Path:  kimiPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_KIMI_MODEL")),
		}
	}
	kiroPath := envOrDefault("FOLIO_KIRO_PATH", "kiro-cli")
	if _, err := exec.LookPath(kiroPath); err == nil {
		agents["kiro"] = AgentEntry{
			Path:  kiroPath,
			Model: strings.TrimSpace(os.Getenv("FOLIO_KIRO_MODEL")),
		}
	}
	if len(agents) == 0 {
	...
	claudeArgs, err := shellArgsFromEnv("FOLIO_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("FOLIO_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("FOLIO_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("FOLIO_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("FOLIO_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("FOLIO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	maxConcurrentTasks, err := intFromEnv("FOLIO_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via FOLIO_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("FOLIO_DAEMON_ID"))
	...
	// — e.g. ~/.folio/daemon.id (default) plus ~/.folio/profiles/<x>/
	...
	// the user explicitly pins FOLIO_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("FOLIO_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("FOLIO_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/folio_workspaces or ~/folio_workspaces_<profile>)
	workspacesRoot := strings.TrimSpace(os.Getenv("FOLIO_WORKSPACES_ROOT"))
	...
	if workspacesRoot == "" {
		...
		if err != nil {
			return Config{}, fmt.Errorf("resolve home directory: %w (set FOLIO_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			workspacesRoot = filepath.Join(home, "folio_workspaces_"+profile)
		} else {
			workspacesRoot = filepath.Join(home, "folio_workspaces")
		}
	}
	...
	keepEnv := os.Getenv("FOLIO_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("FOLIO_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("FOLIO_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("FOLIO_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("FOLIO_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("FOLIO_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("FOLIO_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("FOLIO_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid FOLIO_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("FOLIO_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
