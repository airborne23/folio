...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".folio", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	// only source of truth is ~/.folio/daemon.id.
	profileFile := filepath.Join(home, ".folio", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".folio", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".folio", "daemon.id"))
	if err != nil {
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-folio": uuidB} {
		dir := filepath.Join(home, ".folio", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".folio", "profiles", "corrupt")
