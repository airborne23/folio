...
import (
	...
	"github.com/airborne23/folio/server/internal/daemon/execenv"
	"github.com/airborne23/folio/server/internal/daemon/repocache"
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'folio login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'folio login --profile %s'", d.cfg.Profile)
		}
...
// `folio repo checkout` accepts project-only URLs without an extra round
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Folio Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/folio)
...
// release. Used by pollLoop to expose FOLIO_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	// here would make FOLIO_WORKSPACE_ID empty in the agent env, and the
	...
	// `folio repo checkout` would reject project-only URLs that aren't also
	...
	// can call the Folio API and the local daemon (e.g. `folio repo checkout`).
	// FOLIO_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	agentEnv := map[string]string{
		"FOLIO_TOKEN":        d.client.Token(),
		"FOLIO_SERVER_URL":   d.cfg.ServerBaseURL,
		"FOLIO_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"FOLIO_WORKSPACE_ID": task.WorkspaceID,
		"FOLIO_AGENT_NAME":   agentName,
		"FOLIO_AGENT_ID":     task.AgentID,
		"FOLIO_TASK_ID":      task.ID,
		"FOLIO_TASK_SLOT":    strconv.Itoa(slot),
	}
	if task.AutopilotRunID != "" {
		agentEnv["FOLIO_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["FOLIO_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the Folio CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["FOLIO_QUICK_CREATE_TASK_ID"] = task.ID
	}
	// Ensure the Folio CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// folio binary so that `folio` commands in the agent always resolve.
	...
	// then the daemon-wide FOLIO_<PROVIDER>_MODEL env var. If
	...
	result, tools, lastMessage, err := d.executeAndDrain(ctx, backend, prompt, execOpts, taskLog, task.ID)
	if err != nil {
	...
	if result.Status == "failed" && task.PriorSessionID != "" && result.SessionID == "" {
		...
		retryResult, retryTools, retryLastMessage, retryErr := d.executeAndDrain(ctx, backend, prompt, execOpts, taskLog, task.ID)
		if retryErr != nil {
	...
	case "completed":
		...
		if result.Output == "" {
			...
			// whether to chime in, and the channel-prompt explicitly
			// instructs them to "output nothing" when the message wasn't
			// for them. Pre-channel code only ran issue/chat tasks where
			// empty output was always a failure mode (and rewriting to
			// blocked preserved the session for resume); applying that
			// same rule to channel tasks turns intentional silence into
			// "智能体错误" failure bubbles in the UI. Keep the original
			// completed status for channel tasks; let the server-side
			// finalizer drop the empty row instead of inserting a fail.
			if task.ChannelPrompt != "" {
				return TaskResult{
					Status:    "completed",
					Comment:   "",
			...
			// Issue/chat tasks: empty output is an actual problem, but
			// the agent may still have established a real session.
			// Surface as blocked so the next chat turn can resume.
			return TaskResult{
				Status:    "blocked",
		...
		// Channel tasks render Comment as the user-visible message body.
		// result.Output for codex (and similar) is the *concatenation* of
		// every assistant text the agent emitted in the turn — including
		// reasoning narrations like "I'll check the CLI surface…" that
		// preceded the final answer. Replace it with the last assistant
		// message so the channel sees only the final response. Issue/
		// chat tasks keep using full Output (the transcript IS the
		// artifact in those flows).
		comment := result.Output
		if task.ChannelPrompt != "" && lastMessage != "" {
			comment = lastMessage
		}
		return TaskResult{
			Status:    "completed",
...
//
// Returns lastAssistantMessage in addition to result/tools: this is the most
// recent MessageText fragment the agent emitted, *not* the concatenation
// of all of them. Channel tasks render this as the final reply body —
// concatenated Output mixes reasoning steps ("I'll check the CLI…") with
// the actual answer; the user-visible reply should be only the final
// assistant message in the turn. Chat/issue tasks keep using full Output
// (the transcript is the artifact there, not a single answer).
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID string) (agent.Result, int32, string, error) {
	...
	// lastAssistantMessage is overwritten on every MessageText event so by
	// the time the agent ends, it holds the FINAL assistant message
	// (whatever was emitted last after all tool calls). Channel reply
	// renders this; chat/issue keep using the concatenated Output. Using
	// a sync.Mutex here is overkill since MessageText only fires from
	// the single drain goroutine, but matches the surrounding `mu` style
	// and keeps reads from the outer goroutine race-free.
	var lastAssistantMessage string
	var lastAssistantMessageMu sync.Mutex
	go func() {
			...
			case msg, ok := <-session.Messages:
				...
				case agent.MessageText:
					if msg.Content != "" {
						...
						mu.Unlock()
						// Overwrite, not append: each codex agent_message
						// (or claude completed assistant message) is a
						// discrete text turn. Reasoning narrations come
						// before the final answer; keeping only the
						// last leaves the user-visible channel reply
						// free of "let me check the CLI first" style
						// process narration.
						lastAssistantMessageMu.Lock()
						lastAssistantMessage = msg.Content
						lastAssistantMessageMu.Unlock()
					}
	...
	readLastMessage := func() string {
		lastAssistantMessageMu.Lock()
		defer lastAssistantMessageMu.Unlock()
		return lastAssistantMessage
	}
	select {
	case result := <-session.Result:
		return result, toolCount.Load(), readLastMessage(), nil
	case <-drainCtx.Done():
		...
		if errors.Is(drainCtx.Err(), context.Canceled) {
			...
			}, toolCount.Load(), readLastMessage(), nil
		}
		return agent.Result{
			Status: "timeout",
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "FOLIO_") {
		return true
	}
	switch upper {
