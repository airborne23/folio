...
func TestReleaseAssetCandidates(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			goarch:        "arm64",
			wantAssets: []string{
				"folio-cli-1.2.3-darwin-arm64.tar.gz",
				"folio_darwin_arm64.tar.gz",
			},
		},
		{
			...
			goarch:        "amd64",
			wantAssets: []string{
				"folio-cli-1.2.3-linux-amd64.tar.gz",
				"folio_linux_amd64.tar.gz",
			},
		},
		{
			...
			goarch:        "amd64",
			wantAssets: []string{
				"folio-cli-1.2.3-windows-amd64.zip",
				"folio_windows_amd64.zip",
			},
		},
	}
...
func TestFindReleaseAsset(t *testing.T) {
	t.Run("prefers versioned asset when both names exist", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "folio_darwin_amd64.tar.gz", BrowserDownloadURL: "old"},
			{Name: "folio-cli-1.2.3-darwin-amd64.tar.gz", BrowserDownloadURL: "new"},
		}
		...
		if got.Name != "folio-cli-1.2.3-darwin-amd64.tar.gz" {
	...
	t.Run("falls back to legacy asset when versioned is absent", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "folio_linux_amd64.tar.gz", BrowserDownloadURL: "old"},
		}
		...
		if got.Name != "folio_linux_amd64.tar.gz" {
			t.Fatalf("asset mismatch: got %q", got.Name)
		}
	})
