...
const defaultCLIConfigPath = ".folio/config.json"
...
// An empty profile returns the default path (~/.folio/config.json).
// A named profile returns ~/.folio/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".folio", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.folio/. A named profile returns ~/.folio/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	return filepath.Join(home, ".folio", "profiles", profile), nil
}
