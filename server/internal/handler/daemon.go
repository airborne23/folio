...
import (
	...
	"github.com/airborne23/folio/server/internal/daemonws"
	"github.com/airborne23/folio/server/internal/events"
	"github.com/airborne23/folio/server/internal/middleware"
	...
	"github.com/airborne23/folio/server/pkg/redact"
)
...
type DaemonRegisterRequest struct {
	...
	CLIVersion      string   `json:"cli_version"` // Folio CLI version
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
			...
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					...
					for _, row := range rows {
						...
						// so `folio repo checkout` and the meta-skill render
	...
	// Channel-context task: build a structured conversation-history prompt so
	// the agent sees the channel name, recent messages, and the triggering
	// mention without needing a separate API call.
	if task.ChannelID.Valid {
		channelPrompt, wsID := h.buildChannelPrompt(r.Context(), *task)
		resp.ChannelPrompt = channelPrompt
		if resp.WorkspaceID == "" {
	...
	if task.Context != nil && !task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid && !task.ChannelID.Valid {
	...
	// as the only authority for FOLIO_WORKSPACE_ID in the agent env. An
	...
	if resp.WorkspaceID == "" || resp.WorkspaceID != runtimeWorkspaceID {
		...
		slog.Error("task claim: workspace isolation check failed, cancelling task",
			...
			"has_channel", task.ChannelID.Valid,
...
func (h *Handler) StartTask(w http.ResponseWriter, r *http.Request) {
	...
	// Channel-context tasks: deliberately do NOT insert a placeholder here.
	// We used to render an empty agent bubble immediately, but with multiple
	// agents firing in parallel the placeholders appeared in (near-)
	// concurrent created_at order while replies finalized minutes apart.
	// The result was rows visibly jumping around — a slow agent's empty
	// row stayed above a fast agent's complete reply. By only inserting
	// the row at finalize time, rows naturally show in completion order
	// and the user never sees the "回复中…" intermediate state.
...
type TaskProgressRequest struct {
	...
	// Chunk carries an incremental text token for channel-context streaming replies.
	// Non-channel tasks ignore this field.
...
func (h *Handler) ReportTaskProgress(w http.ResponseWriter, r *http.Request) {
	...
	// Channel-context tasks no longer use streaming chunks. The agent's
	// final answer ships as one shot via CompleteTask's `output` field;
	// req.Chunk for channel tasks is silently ignored.
...
func (h *Handler) CompleteTask(w http.ResponseWriter, r *http.Request) {
	...
	// Channel-context task: finalize the streaming placeholder as complete and
	// broadcast channel:message:completed; also bump agent member stats.
	// req.Output is the agent's final answer text (last assistant message in
	// the stream); we pass it through so finalize writes it as the message
	// body in one shot. Streaming tokens were never forwarded — that path
	// would expose reasoning narration like "Let me check..." in the visible
	// reply, which is what we want to avoid.
	if task.ChannelID.Valid {
		if wsID := h.channelWorkspaceID(r.Context(), *task); wsID != "" {
			h.finalizeChannelStreaming(r.Context(), *task, wsID, true, "", req.Output)
		}
	}
...
func (h *Handler) FailTask(w http.ResponseWriter, r *http.Request) {
	...
	// Channel-context task: finalize the streaming placeholder as failed and
	// broadcast channel:message:completed with the failure_reason.
	if task.ChannelID.Valid {
		if wsID := h.channelWorkspaceID(r.Context(), *task); wsID != "" {
			channelReason := mapDaemonFailureToChannelReason(req.FailureReason)
			h.finalizeChannelStreaming(r.Context(), *task, wsID, false, channelReason, "")
		}
	}
...
// channelTaskContext holds the JSONB payload attached by the channel dispatcher.
type channelTaskContext struct {
...
// buildChannelPrompt builds the agent prompt for a channel-context task and
// returns the rendered string plus the workspace ID (for the isolation check).
// All errors are logged and demoted to warnings — a partial prompt is better
// than a hard failure that cancels a legitimate task.
func (h *Handler) buildChannelPrompt(ctx context.Context, task db.AgentTaskQueue) (prompt string, workspaceID string) {
	var taskCtx channelTaskContext
	if len(task.Context) > 0 {
		_ = json.Unmarshal(task.Context, &taskCtx)
	}

	// Agent display info (name + workspace slug + workspace_id) in one
	// round-trip. workspace_id is required for the downstream isolation check;
	// if this query fails we cannot safely produce a prompt — return empty so
	// the caller's "workspaceID == \"\"" branch cancels the task with a clear
	// signal rather than letting a half-built prompt slip through.
	agentInfo, err := h.Queries.GetAgentDisplayInfoForPrompt(ctx, task.AgentID)
	if err != nil {
		slog.Error("channel prompt: get agent display info failed", "agent_id", uuidToString(task.AgentID), "error", err)
		return "", ""
	}
	workspaceID = uuidToString(agentInfo.WorkspaceID)

	// Channel display info.
	chanInfo, err := h.Queries.GetChannelDisplayInfoForPrompt(ctx, task.ChannelID)
	if err != nil {
		slog.Warn("channel prompt: get channel info failed", "channel_id", uuidToString(task.ChannelID), "error", err)
	}

	// Fetch message history: thread scope if parent_message_id is set,
	// otherwise the most recent N main-timeline messages.
	...
	if taskCtx.ParentMessageID != nil && *taskCtx.ParentMessageID != "" {
		parentUUID, parseErr := util.ParseUUID(*taskCtx.ParentMessageID)
		if parseErr == nil {
			threadMsgs, listErr := h.Queries.ListChannelThreadForPrompt(ctx, parentUUID)
			if listErr != nil {
				slog.Warn("channel prompt: list thread failed", "parent_message_id", *taskCtx.ParentMessageID, "error", listErr)
			}
			...
			slog.Warn("channel prompt: invalid parent_message_id", "parent_message_id", *taskCtx.ParentMessageID, "error", parseErr)
		}
	} else {
		mainMsgs, listErr := h.Queries.ListRecentChannelHistory(ctx, db.ListRecentChannelHistoryParams{
			ChannelID: task.ChannelID,
			Limit:     20,
		})
		if listErr != nil {
			slog.Warn("channel prompt: list recent history failed", "channel_id", uuidToString(task.ChannelID), "error", listErr)
		}
	...
	chanName := "(unknown)"
	if chanInfo.Name.Valid && chanInfo.Name.String != "" {
		chanName = chanInfo.Name.String
	}
	fmt.Fprintf(&b, "You are %q participating in channel #%s of workspace %s.\n",
		agentInfo.AgentName,
		chanName,
		agentInfo.WorkspaceSlug,
	)
	if chanInfo.Topic.Valid && chanInfo.Topic.String != "" {
		fmt.Fprintf(&b, "Channel topic: %s\n", chanInfo.Topic.String)
	}
	if taskCtx.ParentMessageID != nil && *taskCtx.ParentMessageID != "" {
		b.WriteString("You are replying inside a thread. Stay scoped to the thread's topic.\n\n")
	} else {
		b.WriteString("Conversation history (oldest first):\n\n")
	}

	for _, m := range history {
		author := m.authorType + ":" + uuidToString(m.authorID)
		ts := ""
...
// ---------------------------------------------------------------------------
// Channel streaming helpers (used by StartTask/ReportTaskProgress/CompleteTask/FailTask)
// ---------------------------------------------------------------------------

// channelPatchThrottler limits how often channel:message:patched WS events are
// emitted during streaming agent replies. Key is "<channel_id>:<message_id>".
// At ~10 Hz (100ms interval) this prevents socket churn on fast token streams.
type channelPatchThrottler struct {
	mu       sync.Mutex
	lastSent map[string]time.Time
	interval time.Duration
}

func newChannelPatchThrottler(interval time.Duration) *channelPatchThrottler {
	return &channelPatchThrottler{
		lastSent: make(map[string]time.Time),
		interval: interval,
	}
}

// Allow returns true when enough time has passed since the last broadcast for
// the given (channelID, messageID) pair. Thread-safe.
func (t *channelPatchThrottler) Allow(channelID, messageID string) bool {
...
// mapDaemonFailureToChannelReason maps the daemon's free-form failure_reason
// string to one of the three values the channel_message schema allows.
// Unrecognised strings (including "") map to "agent_error" for safety.
func mapDaemonFailureToChannelReason(reason string) string {
	switch reason {
...
// channelWorkspaceID returns the workspace ID for a channel-context task.
// This DOES make a DB call — it duplicates the lookup that
// requireDaemonTaskAccess already performed via ResolveTaskWorkspaceID. Each
// progress chunk on the streaming hot path therefore pays two workspace
// lookups instead of one. Threading workspaceID through the helper signature
// (or returning it from requireDaemonTaskAccess) is the proper fix; tracked
// as TODO and out of scope for C.4.
func (h *Handler) channelWorkspaceID(ctx context.Context, task db.AgentTaskQueue) string {
	if !task.ChannelID.Valid {
		return ""
	}
	wsID, err := h.Queries.GetChannelWorkspaceID(ctx, task.ChannelID)
	if err != nil {
		return ""
	}
	return uuidToString(wsID)
}

// channelParentMessageID parses the optional parent_message_id from the task's
// JSONB context, so the streaming placeholder is placed in the right thread.
func channelParentMessageID(task db.AgentTaskQueue) pgtype.UUID {
	if len(task.Context) == 0 {
		return pgtype.UUID{}
	}
	var ctx channelTaskContext
	if err := json.Unmarshal(task.Context, &ctx); err != nil || ctx.ParentMessageID == nil {
		return pgtype.UUID{}
	}
	u, err := util.ParseUUID(*ctx.ParentMessageID)
	if err != nil {
		return pgtype.UUID{}
	}
	return u
...
// finalizeChannelStreaming records the agent's outcome as a single fresh
// channel_message row at task completion. This is the inverse of the older
// "create placeholder on StartTask, UPDATE on finalize" flow — by inserting
// only at finalize, rows naturally appear in completion order rather than
// dispatch order, and the user never sees an empty "thinking…" placeholder.
//
// finalBody: agent's final reply text (last assistant message). When
// succeeded AND finalBody is empty/whitespace-only, no row is inserted at
// all — that's the "agent decided to stay silent" case (e.g. dispatcher
// fired a subscribe-mode agent on a message addressed to someone else).
// On the failed path the row is still inserted with empty body and
// failure_reason set, so the user sees the failure bubble.
func (h *Handler) finalizeChannelStreaming(ctx context.Context, task db.AgentTaskQueue, workspaceID string, succeeded bool, failureReason, finalBody string) {
	body := finalBody
	if succeeded && strings.TrimSpace(body) == "" {
		// Silent agent on the success path — drop quietly.
		return
	}
	...
	inserted, err := h.Queries.InsertAgentChannelMessage(ctx, db.InsertAgentChannelMessageParams{
		ChannelID:       task.ChannelID,
		AuthorID:        task.AgentID,
		Body:            body,
		ParentMessageID: channelParentMessageID(task),
		DeliveryStatus:  status,
		FailureReason:   reason,
		TaskID:          pgtype.UUID{Bytes: task.ID.Bytes, Valid: true},
	})
	if err != nil {
		slog.Warn("channel agent reply: insert failed",
			"task_id", uuidToString(task.ID),
			"status", status,
			"error", err,
		)
	...
	// Same `channel:message:created` event humans use — the row is born
	// final, no separate "completed" transition to broadcast.
	h.Bus.Publish(events.ChannelMessageCreated(workspaceID, inserted))

	if succeeded {
		if err := h.Queries.UpdateAgentMemberAfterReply(ctx, db.UpdateAgentMemberAfterReplyParams{
			ChannelID: task.ChannelID,
			AgentID:   task.AgentID,
		}); err != nil {
			slog.Warn("channel agent reply: update agent member after reply failed",
				"task_id", uuidToString(task.ID),
				"error", err,
			)
		}
		wsUUID, parseErr := util.ParseUUID(workspaceID)
		if parseErr == nil {
			if _, err := h.Queries.IncrementConsecutiveAgentTurns(ctx, db.IncrementConsecutiveAgentTurnsParams{
				ID:          task.ChannelID,
				WorkspaceID: wsUUID,
			}); err != nil {
				slog.Warn("channel agent reply: increment consecutive agent turns failed",
					"task_id", uuidToString(task.ID),
					"error", err,
				)
			}
		...
		// Thread reply: bump the parent's rollup so reply_count / last_reply_at /
		// reply_participants stay accurate when the responder is an agent. Same
		// idempotent path as the human SendChannelMessage rollup; the dedup is
		// keyed on (type='agent', id=agent_id), so a chain of agent replies in
		// one thread keeps reply_participants at length 1 for that agent.
		if inserted.ParentMessageID.Valid {
			rollup, rollupErr := h.Queries.BumpChannelThreadRollup(ctx, db.BumpChannelThreadRollupParams{
				ParticipantType: "agent",
				ParticipantID:   task.AgentID,
				ParentID:        inserted.ParentMessageID,
			})
			if rollupErr != nil {
				slog.Warn("channel agent reply: bump thread rollup failed",
					"task_id", uuidToString(task.ID),
					"parent_id", uuidToString(inserted.ParentMessageID),
					"error", rollupErr,
				)
			} else {
		...
		// Multi-agent conversation: hand the just-inserted agent message to
		// the dispatcher so other subscribe-mode agents can engage. Without
		// this, "agent A replies → agent B never sees it" because the
		// dispatcher used to fire only on the human-driven SendChannelMessage
		// path. Loop safety is held by the gates the dispatcher already
		// applies on agent-authored triggers: per-agent cooldown and the
		// channel-wide consecutive_agent_turns cap (just incremented above),
		// so chains hit a wall after max_consecutive_agent_turns rounds and
		// reset only when a human posts again.
		if h.ChannelDispatcher != nil {
			if err := h.ChannelDispatcher.Dispatch(ctx, inserted); err != nil {
				slog.Warn("channel agent reply: re-dispatch failed",
					"task_id", uuidToString(task.ID),
					"channel_id", uuidToString(task.ChannelID),
					"error", err,
				)
			}
		}
