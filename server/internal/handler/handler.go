...
import (
	...
	"github.com/airborne23/folio/server/internal/auth"
	channelagent "github.com/airborne23/folio/server/internal/agent"
	...
	"github.com/airborne23/folio/server/internal/storage"
...
type Handler struct {
	...
	ChannelDispatcher     *channelagent.ChannelDispatcher
	channelPatchThrottle  *channelPatchThrottler
	cfg                   Config
}
...
func New(queries *db.Queries, txStarter txStarter, hub *realtime.Hub, bus *events.Bus, emailService *service.EmailService, store storage.Storage, cfSigner *auth.CloudFrontSigner, analyticsClient analytics.Client, cfg Config, daemonHubs ...*daemonws.Hub) *Handler {
	...
	enqueuer := channelagent.NewDBTaskEnqueuer(taskSvc)
	return &Handler{
		Queries:               queries,
		DB:                    executor,
		TxStarter:             txStarter,
		...
		ChannelDispatcher:     channelagent.NewChannelDispatcher(queries, enqueuer),
		channelPatchThrottle:  newChannelPatchThrottler(100 * time.Millisecond),
		cfg:                   cfg,
	}
}
