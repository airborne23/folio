...
// `folio issue comment add` call) left the user staring at a "Completed"
// badge with no reply.
...
// insertTestChannelMessage inserts a top-level channel message from a member
// and returns its id. Registers t.Cleanup to delete the row.
func insertTestChannelMessage(t *testing.T, channelID, memberID, body string) string {
	t.Helper()
	var msgID string
	err := testPool.QueryRow(context.Background(), `
		INSERT INTO channel_message (channel_id, author_type, author_id, body, delivery_status)
		VALUES ($1, 'member', $2, $3, 'complete')
		RETURNING id
	`, channelID, memberID, body).Scan(&msgID)
	if err != nil {
		t.Fatalf("insertTestChannelMessage: %v", err)
	}
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM channel_message WHERE id = $1`, msgID)
	})
	return msgID
}

// insertTestChannelMessageWithParent inserts a thread reply and returns its id.
func insertTestChannelMessageWithParent(t *testing.T, channelID, memberID, body, parentID string) string {
	t.Helper()
	var msgID string
	err := testPool.QueryRow(context.Background(), `
		INSERT INTO channel_message (channel_id, author_type, author_id, body, parent_message_id, delivery_status)
		VALUES ($1, 'member', $2, $3, $4, 'complete')
		RETURNING id
	`, channelID, memberID, body, parentID).Scan(&msgID)
	if err != nil {
		t.Fatalf("insertTestChannelMessageWithParent: %v", err)
	}
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM channel_message WHERE id = $1`, msgID)
	})
...
// TestClaimTask_ChannelPrompt_MainTimeline verifies that ClaimTaskByRuntime
// populates channel_prompt with the recent main-timeline messages when no
// parent_message_id is set, and that workspace_id is resolved from the agent.
func TestClaimTask_ChannelPrompt_MainTimeline(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	...
	// Get agent + runtime from the test workspace.
	var agentID, runtimeID string
	if err := testPool.QueryRow(ctx,
		`SELECT id, runtime_id FROM agent WHERE workspace_id = $1 LIMIT 1`,
		testWorkspaceID,
	...
	// Seed a public channel and a member.
	ch := mustCreateChannel(t, "claim-ch-main", "public")
	channelID := ch["id"].(string)

	// Get the test member id.
	var memberID string
	if err := testPool.QueryRow(ctx,
		`SELECT id FROM member WHERE workspace_id = $1 LIMIT 1`,
		testWorkspaceID,
	).Scan(&memberID); err != nil {
		t.Fatalf("get member: %v", err)
	}

	// Seed 3 messages in order.
	insertTestChannelMessage(t, channelID, memberID, "first message")
	insertTestChannelMessage(t, channelID, memberID, "second message")
	triggerID := insertTestChannelMessage(t, channelID, memberID, "@agent please look")

	// Build context JSONB as the channel dispatcher would.
	...
	var taskID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO agent_task_queue (agent_id, runtime_id, channel_id, status, priority, context)
		VALUES ($1, $2, $3, 'queued', 0, $4)
		RETURNING id
	`, agentID, runtimeID, channelID, taskCtxJSON).Scan(&taskID); err != nil {
		t.Fatalf("create channel task: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(ctx, `DELETE FROM agent_task_queue WHERE id = $1`, taskID) })

	w := httptest.NewRecorder()
	req := newDaemonTokenRequest("POST", "/api/daemon/runtimes/"+runtimeID+"/claim", nil,
		testWorkspaceID, "test-daemon-ch-main")
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("runtimeId", runtimeID)
	req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))
	...
	var resp struct {
		Task *struct {
			...
			ChannelPrompt string `json:"channel_prompt"`
		} `json:"task"`
	}
	if err := json.NewDecoder(w.Body).Decode(&resp); err != nil {
		t.Fatalf("decode response: %v", err)
	...
	if resp.Task.WorkspaceID != testWorkspaceID {
		t.Errorf("workspace_id: want %q, got %q", testWorkspaceID, resp.Task.WorkspaceID)
	}
	if resp.Task.ChannelID != channelID {
		t.Errorf("channel_id: want %q, got %q", channelID, resp.Task.ChannelID)
	}
	for _, want := range []string{"first message", "second message", "@agent please look", triggerID, "channel #claim-ch-main"} {
		if !strings.Contains(resp.Task.ChannelPrompt, want) {
			t.Errorf("channel_prompt missing %q:\n%s", want, resp.Task.ChannelPrompt)
		}
	}
}

// TestClaimTask_ChannelPrompt_ThreadOnly verifies that when parent_message_id
// is set, only the thread's messages appear in channel_prompt and main-timeline
// noise is excluded.
func TestClaimTask_ChannelPrompt_ThreadOnly(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	...
	ch := mustCreateChannel(t, "claim-ch-thread", "public")
	channelID := ch["id"].(string)
	...
	parentID := insertTestChannelMessage(t, channelID, memberID, "parent thread message")
	// Thread replies.
	insertTestChannelMessageWithParent(t, channelID, memberID, "thread reply A", parentID)
	triggerID := insertTestChannelMessageWithParent(t, channelID, memberID, "@agent thread reply B", parentID)
	// Main-timeline noise — must NOT appear in the thread-scoped prompt.
	insertTestChannelMessage(t, channelID, memberID, "main timeline noise")
	...
	if err := testPool.QueryRow(ctx, `
		...
		RETURNING id
	`, agentID, runtimeID, channelID, taskCtxJSON).Scan(&taskID); err != nil {
		t.Fatalf("create thread channel task: %v", err)
	}
	t.Cleanup(func() { testPool.Exec(ctx, `DELETE FROM agent_task_queue WHERE id = $1`, taskID) })

	w := httptest.NewRecorder()
	req := newDaemonTokenRequest("POST", "/api/daemon/runtimes/"+runtimeID+"/claim", nil,
		testWorkspaceID, "test-daemon-ch-thread")
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("runtimeId", runtimeID)
	req = req.WithContext(context.WithValue(req.Context(), chi.RouteCtxKey, rctx))
	...
	prompt := resp.Task.ChannelPrompt
	for _, want := range []string{"parent thread message", "thread reply A", "@agent thread reply B", triggerID} {
		if !strings.Contains(prompt, want) {
			t.Errorf("thread prompt missing %q:\n%s", want, prompt)
		}
	}
	if strings.Contains(prompt, "main timeline noise") {
		t.Errorf("thread prompt leaked main-timeline noise:\n%s", prompt)
	}
	if !strings.Contains(prompt, "thread") {
		t.Errorf("thread prompt missing thread scope indicator:\n%s", prompt)
	}
}
