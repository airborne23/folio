...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	// Quick-create stamp: when the daemon sets FOLIO_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `folio issue create` call
	...
	if taskID := os.Getenv("FOLIO_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set FOLIO_WORKSPACE_ID")
	}
...
// hold a UUID (e.g. agents reading IDs from `folio workspace members
