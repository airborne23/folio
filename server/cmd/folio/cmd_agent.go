...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, FOLIO_SERVER_URL env, or 'folio config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("FOLIO_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("FOLIO_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "FOLIO_SERVER_URL", "")
	if val != "" {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'folio setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
// inside a daemon-managed agent task (daemon sets FOLIO_AGENT_ID and
// FOLIO_TASK_ID in the agent env). In that context the workspace must be
// provided explicitly by the daemon — falling back to user-global
// ~/.folio/config.json would let the agent act on whatever workspace the
...
func inAgentExecutionContext() bool {
	return os.Getenv("FOLIO_AGENT_ID") != "" || os.Getenv("FOLIO_TASK_ID") != ""
}

func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "FOLIO_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inAgentExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: FOLIO_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set FOLIO_WORKSPACE_ID env, or run 'folio config set workspace_id <id>'")
	}
	return id, nil
}
