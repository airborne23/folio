...
// agent task (FOLIO_AGENT_ID / FOLIO_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.folio/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("outside agent context falls back to config", func(t *testing.T) {
		t.Setenv("FOLIO_AGENT_ID", "")
		t.Setenv("FOLIO_TASK_ID", "")
		t.Setenv("FOLIO_WORKSPACE_ID", "")
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("FOLIO_AGENT_ID", "agent-123")
		t.Setenv("FOLIO_TASK_ID", "task-456")
		t.Setenv("FOLIO_WORKSPACE_ID", "env-ws")
...
func TestAgentUpdateNoFieldsErrorMentionsAllCustomEnvFlags(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("FOLIO_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("FOLIO_WORKSPACE_ID", "test-ws")
	t.Setenv("FOLIO_TOKEN", "test-token")
	t.Setenv("FOLIO_AGENT_ID", "")
	t.Setenv("FOLIO_TASK_ID", "")
...
func TestAgentAvatarHappyPath(t *testing.T) {
	...
	t.Setenv("FOLIO_SERVER_URL", srv.URL)
	t.Setenv("FOLIO_WORKSPACE_ID", "ws-1")
	t.Setenv("FOLIO_TOKEN", "test-token")
