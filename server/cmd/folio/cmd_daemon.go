...
import (
	...
	"github.com/airborne23/folio/server/internal/daemon"
	logger_pkg "github.com/airborne23/folio/server/internal/logger"
)
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: FOLIO_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: FOLIO_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: FOLIO_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: FOLIO_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: FOLIO_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Per-task timeout (env: FOLIO_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: FOLIO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: FOLIO_DAEMON_MAX_CONCURRENT_TASKS)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: FOLIO_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: FOLIO_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: FOLIO_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: FOLIO_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: FOLIO_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Per-task timeout (env: FOLIO_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: FOLIO_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: FOLIO_DAEMON_MAX_CONCURRENT_TASKS)")
...
// Empty profile → ~/.folio/, named profile → ~/.folio/profiles/<name>/.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnv(cmd, "server-url", "FOLIO_SERVER_URL", "")
	if serverURL == "" {
	...
	cfg.LaunchedBy = os.Getenv("FOLIO_LAUNCHED_BY")
