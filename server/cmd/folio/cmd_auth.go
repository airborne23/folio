...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate folio with Folio",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("FOLIO_TOKEN")); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'folio setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'folio login' to authenticate.")
		return nil
	}
...
<title>Folio — Authenticated</title>
<style>
