...
func tryResolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"FOLIO_APP_URL", "FRONTEND_ORIGIN"} {
...
var loginCmd = &cobra.Command{
	...
	Long:  "Log in to Folio, then automatically discover and watch all your workspaces.",
	// Up to one positional is accepted so `--token fol_...` (space form) can
...
// "prompt me interactively", preserving the legacy `folio login --token`
// no-value form alongside the documented `--token fol_...` value form.
const tokenPromptSentinel = "\x00prompt"

func init() {
	loginCmd.Flags().String("token", "", "Authenticate using a personal access token. Pass `--token fol_...` to supply it inline, or `--token` alone to be prompted interactively.")
	// NoOptDefVal lets `--token` (no value) keep its old prompt-mode behavior
	// while `--token fol_...` and `--token=fol_...` consume the value normally.
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if err := autoWatchWorkspaces(cmd); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'folio workspace list' and 'folio workspace watch <id>' to set up manually.\n")
		return nil
	}

	fmt.Fprintf(os.Stderr, "\n→ Run 'folio daemon start' to start your local agent runtime.\n")
	return nil
}
...
func waitForWorkspaceCreation(cmd *cobra.Command, client *cli.APIClient) ([]struct {
	...
	if appURL == "" {
		...
		fmt.Fprintln(os.Stderr, "\nNo workspaces found.")
		fmt.Fprintln(os.Stderr, "Create a workspace in the web dashboard, then run 'folio login' again.")
		return nil, nil
	}
