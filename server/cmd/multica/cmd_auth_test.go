...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers FOLIO_APP_URL", func(t *testing.T) {
		t.Setenv("FOLIO_APP_URL", "http://localhost:14000")
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `folio login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
