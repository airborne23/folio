...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("FOLIO_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("FOLIO_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("FOLIO_WORKSPACE_ID")
	agentName := os.Getenv("FOLIO_AGENT_NAME")
	taskID := os.Getenv("FOLIO_TASK_ID")
