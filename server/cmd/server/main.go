...
func main() {
	...
	if os.Getenv("FOLIO_DEV_VERIFICATION_CODE") != "" {
		if strings.EqualFold(strings.TrimSpace(os.Getenv("APP_ENV")), "production") {
			slog.Warn("FOLIO_DEV_VERIFICATION_CODE is set but ignored because APP_ENV=production.")
		} else {
			slog.Warn("FOLIO_DEV_VERIFICATION_CODE is enabled. Use it only for local development or private test instances.")
		}
	}
