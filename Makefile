.PHONY: help makehelp dev server daemon cli folio build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop
...
FOLIO_APP_URL ?= $(FRONTEND_ORIGIN)
DATABASE_URL ?= postgres://$(POSTGRES_USER):$(POSTGRES_PASSWORD)@localhost:$(POSTGRES_PORT)/$(POSTGRES_DB)?sslmode=disable
NEXT_PUBLIC_API_URL ?= http://localhost:$(PORT)
NEXT_PUBLIC_WS_URL ?= ws://localhost:$(PORT)/ws
GOOGLE_REDIRECT_URI ?= $(FRONTEND_ORIGIN)/auth/callback
FOLIO_SERVER_URL ?= ws://localhost:$(PORT)/ws
...
selfhost: ## Create .env if needed, then pull and start the official self-hosted images
	...
	@echo "==> Pulling official Folio images..."
	@if ! docker compose -f docker-compose.selfhost.yml pull; then \
		echo ""; \
		echo "Official images for tag '$${FOLIO_IMAGE_TAG:-latest}' are not published yet."; \
		echo "If this is before the first GHCR release, build from the current checkout:"; \
		echo "  make selfhost-build"; \
		exit 1; \
	fi
	@echo "==> Starting Folio via Docker Compose..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo ""; \
		echo "Images: $${FOLIO_BACKEND_IMAGE:-ghcr.io/folio-ai/folio-backend}:$${FOLIO_IMAGE_TAG:-latest}"; \
		echo "        $${FOLIO_WEB_IMAGE:-ghcr.io/folio-ai/folio-web}:$${FOLIO_IMAGE_TAG:-latest}"; \
		echo ""; \
		echo "Log in: configure RESEND_API_KEY in .env for email codes,"; \
		echo "        or read the generated code from backend logs when Resend is unset."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install folio-ai/tap/folio"; \
		echo "  folio setup self-host"; \
	else \
		echo ""; \
...
selfhost-build: ## Build backend/web from the current checkout and start the self-hosted stack
	...
	@echo "==> Building Folio from the current checkout..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo "Local tags: folio-backend:dev and folio-web:dev."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install folio-ai/tap/folio"; \
		echo "  folio setup self-host"; \
...
daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) folio FOLIO_ARGS="daemon restart --profile local"

cli: ## Run the folio CLI with ARGS or FOLIO_ARGS from source
	@$(MAKE) folio FOLIO_ARGS="$(FOLIO_ARGS)"

folio: ## Run the folio CLI entrypoint directly from the Go source tree
	cd server && go run ./cmd/folio $(FOLIO_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/folio ./cmd/folio
