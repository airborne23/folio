...
import { useIsMobile } from "@folio/ui/hooks/use-mobile"
...
import {
  ...
  SheetHeader,
  SheetTitle,
} from "@folio/ui/components/ui/sheet"
import { Skeleton } from "@folio/ui/components/ui/skeleton"
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "@folio/ui/components/ui/tooltip"
...
function SidebarGroupLabel({
  ...
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          // Anthropic-style editorial group label: uppercase, tracked-out,
          // smaller, no rounded chrome — reads as a magazine section heading
          // rather than a clickable row.
          "flex h-7 shrink-0 items-center px-2 text-[10px] font-medium uppercase tracking-[0.18em] text-sidebar-foreground/50 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-7 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
          className
        ),
      },
      props
...
const sidebarMenuButtonVariants = cva(
  // Anthropic-editorial defaults: hover lifts text colour without flooding
  // the row with a fill; active state uses a deeper bg + a 2px caramel
  // left bar (inset shadow, no layout shift) so the active indicator
  // reads as a marginalia tick rather than a tab. SVG default size is
  // 20px (size-5) — icons do the visual heavy lifting. Icon colour is
  // intentionally NOT overridden here — each consumer paints its own
  // dopamine hue via the className prop, and the colour persists across
  // hover / active states so the per-row identity stays stable.
  "peer/menu-button group/menu-button flex w-full items-center gap-2.5 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[color,background-color,box-shadow,width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent/60 hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent/80 active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:bg-sidebar-accent/70 data-open:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground data-active:shadow-[inset_2px_0_0_var(--color-brand)] [&_svg]:size-5 [&_svg]:shrink-0 [&_svg]:transition-transform [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
        // Default variant inherits the editorial hover from the base — no
        // override needed, otherwise we lose the bg-sidebar-accent/50
        // softening defined above.
        default: "",
        outline:
          "bg-background shadow-[0_0_0_1px_var(--color-sidebar-border)] hover:bg-sidebar-accent/50 hover:text-sidebar-accent-foreground hover:shadow-[0_0_0_1px_var(--color-sidebar-accent)]",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
