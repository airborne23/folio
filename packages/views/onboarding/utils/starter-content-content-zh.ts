...
export const PROJECT = {
  title: "上手指南",
  description: "几件可以在 Folio 里上手试一试的事，按你的节奏走。",
};
...
export function buildWelcomeIssueText(
  ...
  const header = [
    `欢迎来到 Folio！👋`,
    ``,
    `这是你工作区里的第一个 issue。下面你的智能体马上会回复——这就是 Folio 里工作的方式：你写下你想做的事，智能体（或同事）接手并在评论里回复。`,
    ``,
    `[了解 Folio 是怎么运转的 →](https://folio.ai/docs/zh/how-folio-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 欢迎来到 Folio —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 Folio。${name} 主要会让你做 **编码相关的工作**。\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 欢迎来到 Folio —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 Folio。${name} 希望你帮忙做 **规划与拆解工作**。\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 欢迎来到 Folio —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 Folio。${name} 会让你做 **调研和写作** —— 起草、摘要、分析。\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 欢迎来到 Folio —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 Folio。${name} 还在 **探索** Folio 能做什么 —— 暂时没有具体目标。\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 欢迎来到 Folio —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 Folio。${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 欢迎来到 Folio —— 一起开工",
        description: `${header}你好智能体，这是 ${name} 第一次用 Folio。\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**每个 issue 右侧都有一个 Properties 面板**。从这里控制谁来做什么。Folio 里的智能体被触发的条件是 issue 同时满足：`,
        ``,
        ...
        ``,
        `[关于把 issue 分配给智能体 →](https://folio.ai/docs/zh/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[关于聊天 →](https://folio.ai/docs/zh/chat)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[关于工作区 →](https://folio.ai/docs/zh/workspaces)`,
      ].join("\n"),
    },
  ];
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        ...
        ``,
        `[关于成员与角色 →](https://folio.ai/docs/zh/members-roles)`,
      ].join("\n"),
    });
  }
  ...
  tier2.push({
    ...
    description: [
      ...
      ``,
      `[关于创建智能体 →](https://folio.ai/docs/zh/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      description: [
        ...
        ``,
        `[关于执行任务 →](https://folio.ai/docs/zh/tasks)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[关于收件箱 →](https://folio.ai/docs/zh/inbox)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[关于自动化 →](https://folio.ai/docs/zh/autopilots)`,
      ].join("\n"),
    },
  ];
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. 去 [github.com/airborne23/folio/releases/latest](https://github.com/airborne23/folio/releases/latest) 下载 macOS 的 \`.dmg\``,
        `2. 安装并打开`,
        ...
        ``,
        `[关于守护进程与运行时 →](https://folio.ai/docs/zh/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
