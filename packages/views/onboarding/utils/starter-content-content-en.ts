import type { QuestionnaireAnswers } from "@folio/core/onboarding";
import type { ImportStarterIssuePayload } from "@folio/core/api";
...
export const PROJECT = {
  ...
  description:
    "A few things to try in Folio. Work through them at your own pace.",
};
...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[Learn how Folio works →](https://folio.ai/docs/how-folio-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in Folio**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to Folio — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Folio. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to Folio — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Folio. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to Folio — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Folio. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to Folio — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Folio. They're **exploring** what Folio can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to Folio — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Folio. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to Folio — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Folio.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in Folio are triggered when an issue has:`,
        ``,
        ...
        ``,
        `[Learn about assigning issues →](https://folio.ai/docs/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about chat →](https://folio.ai/docs/chat)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about workspaces →](https://folio.ai/docs/workspaces)`,
      ].join("\n"),
    },
  ];
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `Folio works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
        ...
        ``,
        `[Learn about members and roles →](https://folio.ai/docs/members-roles)`,
      ].join("\n"),
    });
  }
  ...
  tier2.push({
    ...
    description: [
      ...
      ``,
      `[Learn about creating agents →](https://folio.ai/docs/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      description: [
        ...
        ``,
        `[Learn about tasks →](https://folio.ai/docs/tasks)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about the inbox →](https://folio.ai/docs/inbox)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about autopilots →](https://folio.ai/docs/autopilots)`,
      ].join("\n"),
    },
  ];
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. Go to [github.com/airborne23/folio/releases/latest](https://github.com/airborne23/folio/releases/latest) and download the \`.dmg\` for macOS`,
        ...
        ``,
        `[Learn about runtimes →](https://folio.ai/docs/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
