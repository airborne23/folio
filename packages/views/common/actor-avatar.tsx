...
import { ActorAvatar as ActorAvatarBase } from "@folio/ui/components/common/actor-avatar";
import {
  HoverCard,
  HoverCardTrigger,
  HoverCardContent,
} from "@folio/ui/components/ui/hover-card";
import { useActorName } from "@folio/core/workspace/hooks";
import { useAgentPresenceDetail } from "@folio/core/agents";
...
export function ActorAvatar({
  ...
  const avatar = (
    <ActorAvatarBase
      ...
      // Pass actorId as the identicon seed so any actor without an uploaded
      // avatar gets the unified marble disc (same across sidebar / channels
      // / detail pages). Once `avatar_url` is set on the actor record, the
      // primitive switches to the image automatically.
      seedId={actorId}
      size={size}
      className={className}
    />
