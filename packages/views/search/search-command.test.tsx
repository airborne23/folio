...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.folio/${p}`),
  mockWorkspaces: {
...
vi.mock("@folio/ui/components/common/theme-provider", () => ({
...
describe("SearchCommand", () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.folio/${p}`);
  ...
  it("copies issue link and identifier when on an issue detail route", async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith("https://app.folio//ws-test/issues/issue-1");
