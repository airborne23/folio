  ...
  useEffect(() => {
    ...
    if (cookieAuth) {
      ...
      Promise.all([api.getMe(), api.listWorkspaces()])
        ...
        .catch((err) => {
          // 401 here just means "no session cookie yet" — the user is
          // anonymous and the caller will render the login page. Logging
          // that as an error every cold-load floods the console and
          // trips Next.js's red overlay. Genuine failures (5xx, network)
          // stay at error level.
          if (err instanceof ApiError && err.status === 401) {
            logger.debug("cookie auth init: no session", { error: err.message });
          } else {
    ...
    const token = storage.getItem("folio_token");
    if (!token) {
      onLogout?.();
    ...
    Promise.all([api.getMe(), api.listWorkspaces()])
      ...
      .catch((err) => {
        ...
        storage.removeItem("folio_token");
        onAuthFailure();
      });
  }, []);
