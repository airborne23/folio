...
export class ApiClient {
  ...
  private readCsrfToken(): string | null {
    ...
    const match = document.cookie
      .split("; ")
      .find((c) => c.startsWith("folio_csrf="));
  ...
  private async fetch<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      // 401 / 403 / 404 are common, expected outcomes — `auth store` calls
      // `getMe()` on every initialize to probe for a session (401 when
      // logged out), workspace list / endpoint permission checks fire 403
      // for users who haven't joined / lost permissions, and 404 covers
      // entities a stale cache references after deletion. Logging any of
      // these at error level pollutes the dev console and trips Next.js's
      // red-screen overlay during normal traffic. Demote them all to warn;
      // genuine server errors (5xx, network) still surface as error.
      const logLevel =
   ...
   * @folio/views/onboarding/utils/starter-content-templates) and are
  ...
  async putChannelMember(
    channelId: string,
    memberRef: string,
    body: { subscribe_mode?: SubscribeMode },
  ): Promise<ChannelMember> {
    return this.fetch(`/api/channels/${channelId}/members/${memberRef}`, {
      method: "PUT",
      body: JSON.stringify(body),
    });
  }

  async removeChannelMember(channelId: string, memberRef: string): Promise<void> {
    await this.fetch(`/api/channels/${channelId}/members/${memberRef}`, {
      method: "DELETE",
    });
  }
  ...
  async listChannelMessages(
    ...
    return this.fetch(`/api/channels/${channelId}/messages${suffix}`);
  }
  ...
  async getChannelThread(
    ...
    return this.fetch(`/api/channels/${channelId}/threads/${parentId}`);
  }
