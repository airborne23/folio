...
This guide documents the local development workflow for contributors working on the Folio codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=folio`
...
POSTGRES_DB=folio
POSTGRES_PORT=5432
DATABASE_URL=postgres://folio:folio@localhost:5432/folio?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=folio_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://folio:folio@localhost:5432/folio_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../folio-feature -b feat/my-change main
cd ../folio-feature
make dev
```
...
The daemon authenticates using the CLI's stored token (`folio login`).
...
whatever server is configured in `~/.folio/config.json`. That's fine for
...
Example: worktree at `../folio-feat-auth` produces profile
`dev-folio_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `FOLIO_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.folio/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `folio` commands automatically use the same
...
rm -rf "$HOME/.folio/profiles/$PROFILE"
```
...
1. Compiles the `folio` CLI from `server/cmd/folio` into
   `apps/desktop/resources/bin/folio`
...
backend logs. If you set `FOLIO_DEV_VERIFICATION_CODE=888888` before starting
...
Nothing in this flow touches the system-installed `folio` or the default
`~/.folio/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.folio/config.json` | `~/.folio/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.folio/daemon.pid` | `~/.folio/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/folio_workspaces/` | `~/folio_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `folio_<slug>_<hash>` |
| Desktop profile | `desktop-api.folio.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U folio -d postgres -At -c "select datname from pg_database order by datname;"
```
