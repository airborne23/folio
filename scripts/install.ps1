# Folio installer for Windows — one command to get started.
#
# Install CLI (default): connects to folio.ai
#   irm https://raw.githubusercontent.com/folio-ai/folio/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Folio server + installs CLI + configures
#   $env:FOLIO_MODE="local"; irm https://raw.githubusercontent.com/folio-ai/folio/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/folio-ai/folio.git"
$RepoWebUrl    = "https://github.com/folio-ai/folio"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".folio\server"
$InstallDir    = if ($env:FOLIO_INSTALL_DIR) { $env:FOLIO_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/folio-ai/folio/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:FOLIO_SELFHOST_REF) {
        return $env:FOLIO_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = folio version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Folio CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Folio requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/folio-ai/folio/releases/download/$latest/folio-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "folio-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "folio.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/folio-ai/folio/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "folio.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "folio-cli-$version-windows-$arch.zip"
        $legacyAsset = "folio_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "folio.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".folio\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "folio.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "folio.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "folio.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "folio.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Folio CLI installed to $binDir\folio.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "folio") {
        ...
        if ($isUpToDate) {
            Write-Ok "Folio CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Folio CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Folio CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "folio")) {
        Write-Fail "CLI installed but 'folio' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Folio self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:FOLIO_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Folio server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Folio repository..."
    ...
    Write-Info "Pulling official Folio images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Folio services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Folio server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Folio - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Folio CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     folio setup               " -NoNewline; Write-Host "# Connect to Folio Cloud (folio.ai)" -ForegroundColor DarkGray
    Write-Host "     folio setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:FOLIO_MODE="with-server"; irm https://raw.githubusercontent.com/folio-ai/folio/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Folio - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Folio server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     folio setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:FOLIO_MODE="stop"; irm https://raw.githubusercontent.com/folio-ai/folio/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Folio services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Folio installation found at $InstallDir"
    }

    if (Test-CommandExists "folio") {
        try {
            folio daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:FOLIO_MODE) { $env:FOLIO_MODE.ToLower() } else { "default" }
